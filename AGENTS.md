...
- GET-139 explicitly replaces the rejected Level 0 topology, movement contract, HUD architecture, and production-art path. Generic preservation guidance in this file applies only after the new specification has locked the replacement contract. `GDR-ART-019` and `memory-bank/01 MVP/32 GET-205 Reference-Native Layout Contract.md` supersede the later four-block envelope too: current direction is the reference-native v6 mission neighborhood, while its exact seed remains reversible under `OPEN-LAYOUT-007` until requester approval of the plan greybox.
