...
const STRINGS: Record<Locale, UIStrings> = {
  en: {
    ...
    questLog: {
      ...
      primaryProgress: 'Current Beat',
      activeSideQuests: 'Active Side Ops',
    ...
    george: {
      ...
      guidanceIntro: 'George: route is simple. Streetlight first.',
      ...
      sliceGuidance: {
        talkToLira: 'Start with Lira. She is your streetlight; she opens the cache run.',
        waitForNight: (curfewWindow) => `Day is for the brief. The cache route counts under ${curfewWindow}; watch the clock and plan the lane.`,
        nightRoute:
          'Curfew is live. Take the north-west cache lane, cut one camera angle, grab the keypad, then get off the street.',
        recoverAtSafehouse: 'Pressure is bending the room. Transit Node is the quiet tide; step inside, breathe, then return to the lane.',
        returnToLira: 'Keypad secured. Bring it to Lira; she closes the first streetlight.',
        talkToNaila: 'Lira’s streetlight is on. Follow the next clue to Naila; she turns the cache into patrol math.',
        findDatapad: 'Naila needs the datapad. Follow the marked corridor, lift it clean, then come back before the city rewrites the schedule.',
        returnToNaila: 'Datapad secured. Return to Naila while the patrol math is still warm.',
        talkToBrant: 'Naila’s window is open. Follow the next clue to Brant; the courier line is the exit wound.',
        findTransitTokens: 'Sweep the marked transit-token trail. Three tokens, one route, no speeches.',
        returnToBrant: 'Tokens recovered. Return to Brant and close the Level 0 chain.',
        complete: 'Lira, Naila, and Brant are closed. The streetlight is back on; take the recap.',
      },
      missionComplete: (name) => `Mission secured in ${name}. Streetlight is on; Harrow’s grid is a little less certain.`,
    ...
    mission: {
      ...
      accomplishedSubtitle: (levelName: string) => `Streetlight is back on in ${levelName}.`,
      primarySummaryLabel: 'What changed tonight:',
      ...
      sideReminder: 'Loose threads still in district:',
      failedTitle: 'Mission Failed',
      failedSubtitle: (levelName: string) => `${levelName} collapsed under the floodlights.`,
      ...
      failedPressureSummary: ({ tier, value }: { tier: string; value: number }) =>
        `Paranoia ${value}/100 (${tier}). Next run: break camera line-of-sight, hit the Transit Node safehouse, then return to Lira before pressure stacks.`,
      failedRetryHint: 'Restart returns to character creation with a clean Level 0 state. Same city, cleaner pulse.',
