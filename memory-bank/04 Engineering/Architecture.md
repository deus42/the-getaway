...
- **`OpsBriefingsPanel.tsx`**: Canonical player objective surface for Level 0, driven by `selectOpsBriefingModel` and rendered as Primary Progress + Active Side Quests in the default HUD, with Available Side Quests and completed history parked in the expanded overlay.
...
4. Broad generated clutter remains out of the live runtime render loop, but GET-156 reintroduces a narrow `level0Environment` atlas path for curated semantic frames only: surface decals, door facades, a small core prop set, and explicit Level 0 pickup frames (`pickup_keypad`, `pickup_datapad`, `pickup_transit_token`, `pickup_medkit`). The registry lives in `the-getaway/src/content/environment/atlasFrames.ts` so render modules refer to semantic IDs instead of hardcoded frame names.
...
6. `the-getaway/src/game/scenes/MainScene.ts` delegates world rendering orchestration to `the-getaway/src/game/scenes/main/modules/WorldRenderModule.ts`, which now resolves surface context from `EnvironmentComposer`, feeds that context into `the-getaway/src/game/visual/world/TilePainter.ts`, updates the camera clear color from the atmosphere profile, and renders building masses/skyline composition plus quality-budgeted atlas decals/props without changing quests, combat, or AI. GET-139 renders pickup items as dedicated Level 0 atlas sprite props before applying their existing highlight/label presentation, including the Lira route Corporate Keycard as the `pickup_keypad` sprite rather than a generic sign. GET-155 adds a source-art Blender slice to normal Level 0 play: `the-getaway/src/content/environment/get155Level0Slice.ts` owns shared placement/collision anchors, `worldMap.ts` marks those collision-footprint tiles as non-walkable floor, and `WorldRenderModule` renders the matching atlas sprites through the existing depth-managed prop path.
...
3. `the-getaway/src/store/selectors/opsBriefingSelectors.ts` builds `selectOpsBriefingModel`, combining mission progress with quest runtime state, quest definitions (`kind`, related NPCs, mission summaries), and the Level 0 guided-step contract. In GET-139 the default primary list shows completed/started beats plus the current Lira/Naila/Brant beat so the HUD stays one-step-at-a-time.
...
8. `the-getaway/src/game/controllers/MiniMapController.ts` composes objective markers from live items plus the current `level0GuidedSlice` contact, so the minimap emphasizes the active Lira/Naila/Brant beat instead of broadcasting every potential contact at once.
