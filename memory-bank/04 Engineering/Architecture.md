...
5. `WorldRenderModule` fits one manifest-driven painterly compound texture to each runtime footprint and suppresses painterly perimeter volumes/labels through map-profile capabilities
...
- Every painterly actor set preserves `64×96` frames, four frames per animation, eight directions, `idle/move/attack/interact`, the established FPS, origin `0.5/0.92`, and foot-anchor validation. Presentation scales are role-specific: heroes `1.08`, named NPCs `1.00`, and guards/hostiles `1.02`.
- `src/content/environment/level0BuildingArtManifest.ts` is the source of truth for all nine named outdoor buildings: texture path/key, origin, footprint fit, visual height, and fallback profile. Each runtime PNG is a block composite that preserves its landmark while attaching contiguous street-wall wings. `BootScene` preloads the manifest, and `WorldRenderModule` uses the same path for the Empire State Building instead of a renderer-specific exception.
...
- `App.tsx` places `data-visual-style="graphic-painterly-noir"` on the document root only while the exact outdoor Level 0 map is active. `src/styles/hud-theme.css` supplies semantic painterly variables, and `src/App.css` scopes matte treatment across the Level 0 menu/time controls, dock, minimap, George, objectives, dialogue, combat, progression, recap, failure, and retry surfaces without changing layout or breakpoints.
- `CameraModule` opens outdoor Level 0 at a `0.80` neighborhood-scale presentation floor. The existing `0.38` manual zoom-out limit and combat zoom behavior remain unchanged. Painterly map-profile capabilities also suppress permanent building labels and raised perimeter-wall volumes so the normal view reads as a district rather than a bounded board.

### Technical flow

1. `BootScene` preloads character sheets, portrait PNGs, building textures, and both painterly and fallback semantic atlases.
...
5. Day/night and combat systems continue to drive atmosphere, overlays, pathing, and interaction semantics through existing state contracts. Painterly backdrop values stay close to the outer lot mass so the collision boundary does not become a bright floating-board edge.
6. `scripts/validate-level0-painterly-art.ts` validates the nine metric-backed building outputs and both source composite sheets; `sprites:validate` independently validates the actor matrix.
