...
5. `the-getaway/src/game/scenes/BootScene.ts` preloads the shared prop atlas (`props`), ESB atlas, character sheets, and the constrained Level 0 environment atlas/normal sheet. Normal data is attached to the atlas texture after load, matching the existing normal-map path while keeping vector fallback available when frames are missing.
6. `the-getaway/src/game/scenes/MainScene.ts` delegates world rendering orchestration to `the-getaway/src/game/scenes/main/modules/WorldRenderModule.ts`, which now resolves surface context from `EnvironmentComposer`, feeds that context into `the-getaway/src/game/visual/world/TilePainter.ts`, updates the camera clear color from the atmosphere profile, and renders building masses/skyline composition plus quality-budgeted atlas decals/props without changing collision, pathing, quests, combat, or AI.
