...
The approved but not yet runtime-realized GET-205 v6 boundary uses recipe identity `get205-reference-native-v6` and runtime identity `get205-reference-native-production-v3`. [[32 GET-205 Reference-Native Layout Contract]] supplies one frozen plan manifest for bounds, routes, parcel footprints/heights, declared perimeter masses, dynamic gate collision, semantic anchors, the exact hero camera, and the replacement 22-static/four-dynamic probe fixture. Stable mission IDs and runtime/pipeline interfaces survive; old coordinates do not. The requester approved both the dimensioned plan and amended raw greybox on 2026-08-13. The frozen hero camera is target `{21.177121,17.126983,1.621967}`, orthographic scale `25.687085`, 45° azimuth, and 30° elevation; the accepted raw baseline SHA-256 is `ddf5a224da0eec2fe8af5b1c0bf2f7ca8f5e618080ef1d63b4a18c4c72aa7369`. Pure seed projection and rendered mission-legibility masks remain mandatory regression gates during source-facade dressing. The source builder derives a new v6 master without saving over GET-204/v4/v5 donors, creates 27 separate visible Neo Tokyo 2 assemblies, and emits a parcel-complete build manifest containing source collection/object/material identifiers, source and placed bounds, resolved transforms, mesh complexity/hash evidence, authored crop/cap/interior-closure records, clutter source/purpose/clearance records, and image-dependency durability records. Before rendering, it saves and reopens the derivative, rejects every missing unpacked image, and reports whether each retained image is packed or resolves through the durable local licensed cache; an in-memory basename remap is not acceptance evidence. The same builder creates the HIDZU landmark/checkpoint signs as mounted dimensional geometry and emits a frozen-camera glyph-parity preview; camera-facing planes, decals, and mirrored required glyphs fail. A saved-scene validator independently rejects empty source parcels, unsupported provenance claims, camera-visible parcel envelopes/generated facades, geometry outside declared parcels, source-only silhouette IoU below `0.85`, unresolved images, donor-master mutation, or flat HIDZU sign substitutes. Until the real-source blue-hour hero and later live publication gates pass and publication swaps atomically, the realized v4 architecture remains the recoverable runtime baseline and no v6 identity may resolve at runtime.
...
- One versioned v6 plan manifest owns streets, non-rendering parcel envelopes, declared perimeter masses, staging sight wedges, the frozen camera, dynamic gate, anchors, and probes. Pure projection preflight blocks Blender when those seed-level composition gates fail; rendered masks then confirm actual occlusion. The named-source Neo Tokyo 2 Blender master realizes each parcel with visible source architecture; the plan envelope may guide fit/collision/masks but cannot be a camera-visible building shell. Source-only silhouette parity uses IoU `>=0.85` so modeled relief is legal without reopening the frozen footprint/camera contract. Source-facade and live requester acceptance promote that exact reconciled contract into runtime; they do not reopen layout.
...
- V6 source assembly may append from the historical masters, but texture recovery is performed only in the new derivative. The render runner audits image dependencies both before render and after save/reopen, fails on any unresolved external path, and never treats an in-memory relink count as durable success. The current historical donor paths are evidence inputs, not files to rewrite.
- HIDZU tower and checkpoint identity is one project-authored geometry subsystem: mounted steel frame/box, brackets/rails, dimensional hex mark and letters, controlled emissive faces, and small unit plates. A cheap frozen-camera preview verifies forward glyph orientation before the full Cycles render; no post-render, plane, or decal path is supported.
...
- Level 0 exposes a close street-first normal frame and a manual minimum zoom that reaches the complete mission-neighborhood overview. V6 hero translation/scale are frozen by requester greybox approval at target `{21.177121,17.126983,1.621967}` and orthographic scale `25.687085`; old GET-204 target/zoom constants are not inherited. `OPEN-MOV-003` retains only player-follow and runtime zoom-range tuning around this fixed composition.
...
Schedules are authored state tables keyed by world phase/boundary, not free-running NPC scripts. Schedule transitions can change availability, position/path definitions, public/blending context, shutters, crowd density, signage/light state, and ambience. They cannot move a currently interacting actor or mutate geometry silently. GET-205 binds two seated and one standing transit actor to explicit shelter slots and two presentation-only café actors to declared restaurant furniture; the transit group is populated at 18:45, winds down after 21:30, and disables `blend.public_queue` at curfew, while the absent delivery group keeps `blend.delivery_activity` unavailable. Broader movement/reaction behavior remains GET-208-owned. Spatial ambience has three authored world anchors: the Transit Road restaurant, Market Ring workshop, and safehouse-side apartment.
...
6. Preserve the requester-approved reference-native v6 plan, camera, and probes; rebuild it from named Neo Tokyo 2 sources, obtain hero/overview approval, reconcile its frozen geometry into the runtime layout contract, integrate it live, and obtain separate live acceptance; only then may closeout and the implementation commit proceed.
