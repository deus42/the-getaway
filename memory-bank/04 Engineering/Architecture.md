...
15. `the-getaway/src/components/GameCanvas.tsx`, `the-getaway/src/game/settings/visualSettings.ts`, and `the-getaway/src/store/settingsSlice.ts` coordinate smooth-vector renderer defaults and quality-preset-aware FX budgets (`performance | balanced | cinematic`), including shared fog/emissive/wet-reflection/occlusion caps consumed by scene render systems. `the-getaway/src/game/scenes/main/modules/CameraModule.ts`, `the-getaway/src/App.tsx`, `the-getaway/src/styles/hud-bottom-dock.css`, `the-getaway/src/utils/bottomDockSizing.ts`, and `the-getaway/src/game/scenes/main/modules/EntityRenderModule.ts` now also participate in Level 0 visual deconfliction by slightly widening the fit zoom, softening the bottom dock, reducing always-on nameplate intensity, and sizing the dock from live lane content demand so CTA controls are not clipped when the compact shell is active.
