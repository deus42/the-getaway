...
| OPEN-RPG-002 | Critical | Level 0 XP award and first threshold | Award one milestone at medkit return and one at transit validation; total exactly reaches Level 2 during debrief, granting two skill points. | T7, T10 |
...
| OPEN-ART-001 | Critical | Exact Direction B reference artifact | Register the approved image path or create a replacement board; do not begin Hidzu treatment from an unresolvable label. | T5 |
| OPEN-ART-002 | Critical | Exact Neo Tokyo 2 asset selection and license/provenance manifest | Inventory chosen assets, source paths, transformations, and prohibited committed geometry before master-scene work. | T4 |
| OPEN-ART-003 | High | Actor runtime scale and frame occupancy | Start at approximately 15% above strict perspective; accept only after 0.60 and default-zoom screenshots. | T6 |
...
| OPEN-ACC-001 | High | Accessibility baseline | Lock text scaling, reduced motion/flash, volume controls, color-independent risk cues, keyboard parity, and readable subtitles before UI acceptance. | T9, T10 |
| OPEN-PERF-001 | High | Level 0 performance/load budgets | Set target hardware, first-load budget, texture/memory ceiling, and stable frame target before committing tiled Blender derivatives. | T4, T6, T10 |
