...
The approved mission skeleton first defines required places, route purposes, and stable semantic IDs. `GDR-ART-019` and [[32 GET-205 Reference-Native Layout Contract]] now define the current reference-native geography and exact v6 seed. One named-source Neo Tokyo 2 Blender master exports candidate semantic metadata after its plan/greybox and source-visual gates; after live requester acceptance, that reconciled geometry becomes the accepted `Level0LayoutContract` for:
...
The sparse/fenced compound, exact `54×38`, `58×44` GET-204/v5 grid, `84×60` nine-block geometry, `96×72` experiment, and oversized full-district expansion are historical and are not preservation constraints. GET-204, v4, and rejected v5 sources remain immutable and recoverable. V6 preserves semantic IDs, mission beats, route purposes, runtime systems, and the `0.32` clearance policy—not old coordinates. Its `44×38` seed, three-arm junction, real controlled gate, parcel strips, relocated anchors, camera derivation, and replacement 22-static/four-dynamic probe fixture remain reversible only under `OPEN-LAYOUT-007` until the requester accepts the plan greybox.
