...
| Preparation → Departure | Safehouse + persistence | Consult any contacts or neither, review George's exact readback, then confirm departure | Mission accepted; requester-approved v6 departure seed; unsafe actions under `OPEN-SAFE-001` | Naila/Brant facts and optional gates completed or skipped | Real departure time, contacts, the Paranoia tier, held abilities, and Restart Attempt meaning | Immutable `OperationAttemptBaseline` created before later world mutation | `departedAt`, contacts/facts, primary timing begins | [[32 GET-205 Reference-Native Layout Contract]], [[44 Safehouse, Save & Restart Attempt]], [[80 Day-Night Cycle]] / T3, T7, T10, GET-211 |
...
The approved mission skeleton first defines required places, route purposes, and stable semantic IDs. `GDR-ART-019` and [[32 GET-205 Reference-Native Layout Contract]] now define the current reference-native geography and exact v6 seed. One named-source Neo Tokyo 2 Blender master realizes all 27 parcels as visible source assemblies and exports candidate semantic metadata plus parcel-complete collection/object/material/transform/bounds/hash provenance after its plan/greybox and source-visual gates; camera-visible parcel envelopes or generated facades fail. After live requester acceptance, that reconciled geometry becomes the accepted `Level0LayoutContract` for:
...
The sparse/fenced compound, old GET-204/v5 grid, nine-block geometry, larger experiment, and oversized full-district expansion are historical and are not preservation constraints. GET-204, v4, rejected v5, and the first rejected v6 raw frame remain immutable/recoverable evidence. V6 preserves semantic IDs, mission beats, route purposes, runtime systems, and the `0.32` clearance policy—not old coordinates. The requester-approved reference-native core, bounded south-west safehouse extension, three-arm junction, real controlled gate, amended one-floor foreground strips, three sight wedges, relocated anchors, exact hero camera, and replacement 22-static/four-dynamic probe fixture are now frozen by `GDR-ART-019`.
