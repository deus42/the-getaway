...
The player creates a grounded expatriate protagonist who is not already a Hidzu Corporation target, seeks passage toward Miami to investigate their father and Cold Iron, makes optional preparation choices, reads and evades truthful surveillance, recovers Lira's medkits, may copy consequential Cold Iron evidence, validates transit before midnight, and receives a factual debrief. George remains the recognizable recovered cyan-core orb in the world and four-lane HUD—not a generic human assistant. Failure and Restart Attempt teach the real system without invented knowledge, tactical combat, or opaque shortcuts.
...
- Direct requester correction on 2026-08-09 supersedes two later visual-acceptance assumptions: GET-205's committed warm/dry grade is a technical baseline awaiting a blue-black wet Blender rebake, and GET-206's human-bust George is rejected while the grounded human cast remains reusable. `GDR-GEO-005` restores the pre-rewrite orb identity from commit `49a4da7`.
...
Direct click/WASD movement, explicit interaction, full-pause Observation, deterministic gates, binary general facts, the dedicated Cold Iron chain, Paranoia tiers, `Clear/Suspicious/Pursuit`, last-known search, discrete hiding/blending, one Needle patrol, four-lane HUD, and authored world time remain the current contract. Concern requires current visibility plus one of five approved `SurveillanceRuleBreakEvidence` types. Every nonterminal gate failure commits a declared worse path. Clock boundaries at 21:00, 21:30, 22:00, and 23:30 are idempotent. Actor light regions are presentation-only. World art targets blue-black wet reflective surveillance noir, and George's world/HUD state always preserves the recovered orb silhouette.

### Internal milestones and proof gates

1. Validate and separately commit this canonical/Linear correction package.
2. Preserve the top-level T1→T10 dependency program while reopening only the visually superseded acceptance gates.
3. Deliver GET-205's versioned dense-city rebuild and reference-led Blender rebake. Preserve the executable route/anchor/loop contract while allowing dead interstitial block gaps to become building mass; obtain requester approval at the greybox massing gate, blue-hour hero gate, and final live side-by-side gate.
4. Deliver GET-206's recovered George orb against that accepted world while preserving the useful grounded human cast.
5. Deliver T3A Restart foundation, then T8A surveillance and GET-179 modernization; require their milestone before T9A information/failure work.
6. Deliver GET-209's synchronized orb/HUD infrastructure, GET-213's legibility integration, T10A city time/routes/recovery/audio, then T10B actor-light integration in dependency order.
7. Run normal-control, bilingual, fixed-viewport evidence before automated closeout and keep GET-139 nonterminal until the requester verifies the authorized committed build.
...
The street—not only the HUD—communicates time, coverage, route identity, and institutional presence through cool wet materials, motivated practical light, and truthful surveillance cues. Normal camera warnings are subtle light/reflection cues; Observation shows exact discovered geometry. Needle has a hum and authored warnings. Civilians show only small visible reactions. George and current-task lanes remain separate; the same recovered cyan-core orb appears near the protagonist and in his lane. George is factual, explicitly explains information limits, never hides information in silence, and has no deletion/freedom arc. No Observation vignettes or decorative Bible quotations are added.
...
- No George human face/bust/body or alternate-logo replacement; no post-process-only recolor presented as the GET-205 Blender rebake.
...
Implements the full approved decision set, especially GDR-PC-005, GDR-SAFE-001, GDR-SUR-006 through GDR-SUR-010, GDR-CIV-001, GDR-RPG-007, GDR-FACT-002, GDR-PAR-006, GDR-PAR-007, GDR-GEO-004, GDR-GEO-005, GDR-FAIL-001, GDR-TIME-004, GDR-SET-007, GDR-AUD-002, GDR-HLT-003, and GDR-ART-011. Canonical sources are [[Game Design]], [[10 MVP Spine]], [[11 Level 0 Vertical Slice Contract]], [[12 Game Design Decision Register]], [[13 Level 0 Content and State Matrix]], [[14 Specification Review Queue]], all affected 16-section system specifications, [[03 Lore/Plot Bible]], [[04 Engineering/Architecture]], [[04 Engineering/Roadmap]], and [[95 MVP Readiness Checklist]].

### Human-play acceptance

Run the complete matrix at 1280×720, 1440×900, and 1920×1080 in English and Ukrainian. Before systemic closeout, obtain separate live side-by-side acceptance for the blue-black wet GET-205 rebake and the recovered George orb in both world/HUD forms. Then prove harmless public observation; each concern source; geometric blind spots; single camera use/history; Clear reset; Needle warnings; presentation-only civilians; exact gate preview/result verdicts and fail-forward paths; all Cold Iron states/copy cost; baseline restoration; one-use grounding/threshold lines; idempotent clock changes; evidence-limited failure reports; named routes/sound thresholds; and presentation-only actor tint. Use normal controls only.
...
The canonical package specifies graphic surveillance noir, the reference-native mission neighborhood and its three stable route purposes, named Neo Tokyo 2 source provenance, close street-first play and a complete-neighborhood overview, truthful surveillance geometry, the four-lane 16–18% dock, an always-visible named Paranoia tier with lit/locked abilities, world-visible paused overlays, one-function terminals, knowledge-limited minimap, bounded George prompts and private AR presence, and required audio families. It states which internal values remain open instead of turning recommendations into silent production constants.
...
- Current layout dimensions, loop geometry, street widths, anchors, safehouse seed, frozen hero camera, and replacement probe fixture come from the requester-approved [[32 GET-205 Reference-Native Layout Contract]]; actor movement and follow tuning remain separately open. T3's existing geometry is replaceable implementation, not authority.
- The world is outdoor-only; no full interior is required.
- T3 begins from stable mission semantics and measured direct-movement behavior. Its current geometry must be replaced by the reference-native v6 plan; it must not preserve the old `54×38`, `58×44` four-block/v5, `84×60`, `96×72`, nine-parcel, or sprite-collage assumptions.
...
Depends on T2. Blocks T3A, T4, T7, T8, T9, and T10. Current critical acceptance blockers are `OPEN-NAR-007`, `OPEN-TIME-001`, `OPEN-MOV-001`, `OPEN-CIV-001`, `OPEN-SEC-001`, `OPEN-LAYOUT-003`, and `OPEN-SAFE-001`. High blockers are `OPEN-NAR-013`, `OPEN-MOV-002`, and `OPEN-MOV-003`. `OPEN-LAYOUT-007` is resolved by the requester-approved v6 seed, camera, and probe fixture. These remaining items do not block ticket start after T2; a recorded recommendation encoded before approval remains a reversible provisional trial with seam, live proof, and rollback.

### Canonical decisions/spec sections

Implements `GDR-PROD-004`, `GDR-SET-002`, `GDR-MIS-006`, `GDR-TIME-001` through `GDR-TIME-003`, `GDR-MOV-001` through `GDR-MOV-003`, `GDR-INT-001`, `GDR-OBS-001`, `GDR-OBS-002`, `GDR-STL-001`, `GDR-ART-019`, `GDR-REM-003`, `GDR-REM-008`, `GDR-SUP-001`, and `GDR-SUP-002`.
...
Historical GET-204 owns its recoverable named-source four-block evidence, same-master proof, and collision lessons; it no longer owns current Level 0 geography. `GDR-ART-019` and [[32 GET-205 Reference-Native Layout Contract]] replace that geography through T5. Named Neo Tokyo 2 provenance, source/live proof discipline, and `GDR-ART-014` proportion remain current. The requester-approved v6 hero camera is frozen at target `{21.177121,17.126983,1.621967}` and orthographic scale `25.687085`; player-follow tuning remains `OPEN-MOV-003`; performance remains `OPEN-PERF-001`; `GDR-ART-013` fixes the 750 ms state-delivery contract.
...
Four failed GET-205 composition rounds share one cause: the ticket kept trying to dress the 2026-08-05 `58×44` four-block street grid as Reference 2 while preserving route polygons and anchors whose positions the hero camera could not show. Lighting fixed the color family, v5 improved coverage, and mission placeholders improved diagnosis, but none could make inherited geography become the reference's plan. T5 therefore replaces that geography through `GDR-ART-019`: reconstruct the reference's tight three-arm junction, life street, HIDZU-controlled street, real checkpoint, tower, transit/café life, devices, and shadow seams directly, then extend the same grammar into the safehouse quarter, logistics depth, service alley, and one sneak bypass. Stable mission semantics and runtime/pipeline systems survive; old coordinates do not.

### Player promise

The city is the reference's playable mission plan, not another adjacent approximation. One glance shows the life street up-left, the HIDZU destination above a real gate up-right, the transit queue and café, visible terminals, and the shadow bypass. Continuous narrow parcels, detailed warm and cold windows, controlled colored fixtures, one dominant tower, and an actual private boundary make the operation readable by construction. Random ambient allocation is removed, but purposeful public life remains: two seated plus one standing transit passenger, two seated café patrons, one public guard, and Needle, all separately rendered at exact mission slots. The shared look remains wet blue-black surveillance noir with readable cool midtones, continuous rain sheen, crisp broken reflections, localized amber practicals, declared cyan technology/facade accents, and sparse threat-specific red.

### Starting state

- GET-204's named-KitBash source and accepted recipe remain immutable/recoverable. `get205-four-block-baked-v3`, wet-blue-black v4, and every v5 recipe/render/report are historical technical or failure evidence, never current geography.
- V6 uses recipe identity `get205-reference-native-v6` and eventual runtime identity `get205-reference-native-production-v3`. It never mutates or republishes a prior identity in place.
- `GDR-ART-019` explicitly retires the `58×44` grid, four-block invariant, old footprint/route polygons, all frozen anchor coordinates, `{29,22}` camera target, landmark-at-`x=36` mandate, v5 density-ratio authority, and recovered 24-point fixture. Stable semantic IDs, route-loop IDs/names, mission beats, interaction semantics, direct movement, projection family, HUD, actors, three-state delivery, profile selection, occlusion machinery, and persistence survive.
- [[32 GET-205 Reference-Native Layout Contract]] sections 6.1–6.9 supply the mandatory exact frozen seed. The requester approved the dimensioned plan, rejected its first roof-dominated raw frame, authorized the bounded foreground amendment, and approved the amended raw greybox on 2026-08-13 after the blind five-question read. `OPEN-LAYOUT-007` is resolved. The frozen hero camera is target `{21.177121,17.126983,1.621967}`, orthographic scale `25.687085`, 45° azimuth, and 30° elevation; the accepted raw baseline SHA-256 is `ddf5a224da0eec2fe8af5b1c0bf2f7ca8f5e618080ef1d63b4a18c4c72aa7369`. The replacement 22-static/four-dynamic probe fixture is now invariant.
- Reference authority is explicit: `kitbash-reference2-blend-concept-v1.png` owns the literal core street/mission plan, hero composition, source relationship, and human/door/sidewalk/street/building proportions without becoming production geometry. `canvas-quality-target.png` owns wet blue-black material/light/reflection quality. `street-play-target.png` owns normal-camera intimacy, social staging, surveillance readability, and HUD/world balance but not actor size. `dense-city-target.png` remains an overview-density check only.
- V6 invalidates all v4/v5 state masters, cutouts, collision/placement pins, camera registrations, transfer ceilings, regression frames, and runtime identity expectations. Those artifacts are regenerated only after plan and hero approval.
- The first dressed v6 source-facade rounds are rejected hero evidence, not a lock. Read-only diagnosis proved that real Neo Tokyo 2 meshes are present but are almost completely obscured by camera-visible procedural parcel envelopes; hiding only those envelopes reveals the modeled frames, ledges, storefront depth, pipes, and roof equipment. The corrective build preserves the approved plan, camera, mission staging, wet-material relationship, and color discipline while replacing the visible parcel-construction layer.
- The consolidated local mockup bundle is method evidence only. Its replica/transplant scripts prove append, parent-clear, street-face placement, and the visual difference between kit geometry and parcel shells; they do not own production coordinates, camera, road width, donor count, destructive clearing, or signage. Their three-wide-donor life wall, altered set-piece plan, banner/image planes, and decals are explicitly excluded from the v6 build.
- The documentation entry package is committed and read back. The raw-greybox approval requires one separately authorized layout-closure authority commit before source-facade production; no implementation commit is implied by visual approval.
...
- T5 may change all versioned geography and placement only by reproducing or canonically revising the exact v6 seed through the declared requester gates. It may not retain an old coordinate for convenience, alter device/mission semantics, or weaken the replacement route/probe/gate verdicts to fit art.

### Complete player flow

The player leaves the safehouse quarter through its short lane and enters the south approach. At the junction, the life street rises up-left and the HIDZU street rises up-right. The tower is directly above the checkpoint. The barrier spans the controlled road and is a real dynamic navigation boundary: closed before authorization, open only when the existing verification/mission contract allows passage. Private space begins beyond it. Needle's launch/patrol volume and the one public guard remain visibly associated with that gate. The eastern shadow seam permanently bypasses the checkpoint into the service alley and service entrance; it intentionally has no static service guard in this ticket.

The life street stages the transit shelter, bench, timetable fixture, exactly two seated passengers and one standing passenger at `blend.public_queue`, plus a street-level café with exactly two seated patrons. Street-facing fixtures mark all three relocated terminal anchors, and tight recesses mark all three relocated hide anchors so hack and sneak options exist before facade glow or HUD explanation. `blend.delivery_activity` remains unavailable until GET-208 authors its population. All people remain runtime-owned and absent from environment plates. Dusk, blue hour, and curfew change windows, practicals, surveillance emphasis, population schedule, and sparse threat color without moving geometry or hiding routes.

### System rules and state transitions

The T5 lifecycle is `COMMITTED_TECHNICAL_BASELINE → REFERENCE_NATIVE_DOCUMENTATION_COMMIT → EXACT_PLAN_MANIFEST → DIMENSIONED_PLAN_AND_RAW_GREYBOX_GATE → LAYOUT_CLOSURE_AUTHORITY_COMMIT → SOURCE_FACADE_HERO_LOCK → THREE_STATE_EXPORT → CUTOUT_PROFILE_MANIFEST_REGEN → LIVE_SIDE_BY_SIDE_ACCEPTANCE → ACCEPTANCE_COMMIT → COMMITTED_BUILD_VERIFICATION`.

One plan manifest drives bounds, routes, parcels, building footprints, gate collision, anchors, proof starts, the frozen camera, the replacement probe fixture, and declared perimeter masses. Before any source-facade render, projected seed math must retain the amended foreground caps, displaced-safehouse lower-band clearance, tagged-roof span `<=0.18H`, and camera-clear transit/café/sneak target polygons at `>=80/60/2,500` projected pixels with no intersecting declared mass above one floor. The validator also rejects any footprint/fixture that intersects a route or required `0.32` clearance, any static verdict that differs from the 22 frozen outcomes, failure of the four dynamic gate/bypass/reset probes, a disconnected required anchor, or drift from the exact seed/camera without a canonical amendment. Occlusion-aware masks require at least ten hero identities contributing at least `0.005` of the full frame each and none above `0.15`; declared counts and token slivers are invalid. Source-facade dressing must preserve the approved destination, obstacle, blend, hack, and sneak reads; it cannot retarget or rescale the camera. No state bake, cutout, or runtime publication starts until requester blue-hour hero approval.

Every one of the 27 parcels must visibly resolve to an actual Neo Tokyo 2 building or kit-bashed source assembly. The saved scene and manifest name the KB3D source collection, retained source objects/materials, source and placed bounds, resolved transform, mesh complexity/hash evidence, and authored crop/cap/interior-closure work per parcel. A source prefix, texture relink, object count, hidden licensed mesh, camera-visible procedural envelope, emissive decal, painted window grid, or generated facade cannot satisfy provenance. Real source relief is compared with the approved greybox at silhouette IoU `>=0.85`; the tolerance covers modeled facade/roof detail only and never permits a footprint, height rhythm, mission fixture, route, camera, or staging-wedge change.

Historical GET-204/v4/v5 donor masters remain read-only. The new v6 derivative owns every source-image repair: remap each retained image to the durable local licensed cache or pack it into the derivative, save, reopen, and block rendering unless the reopened scene reports zero missing external images. A session-only basename remap or aggregate repaired-link count is diagnostic, not durability proof. HIDZU tower/checkpoint identity is likewise source-level project-authored geometry: mounted frames/boxes, brackets/rails, dark returns, dimensional hex mark/letters, and small unit plates. Banner/image planes, decals, post-render signs, and mirrored required glyphs fail; a low-sample frozen-camera preview must prove readable forward copy before the full hero render.

After layout approval and the closure authority commit, blue hour becomes the visual anchor. Dusk, blue hour, and curfew are three Cycles-baked people-free asset sets from one immutable v6 geometry/camera registration. Blue hour becomes authoritative at 20:00 and curfew at 22:00; each target prefetches at 19:50 and 21:50 and replaces the prior complete set through a 750 ms crossfade only after every target texture is ready. A generation token rejects stale jumps, restarts, and hydration completions. Failed transitions discard the partial target, retain the current complete state, diagnose observably, and retry at the next synchronization or reload; initial-state failure is visibly fatal.

Active technology cyan appears only on declared devices, Hidzu Corporation technology, and a small declared set of non-directional facade identity seams. Cyan wayfinding, route/street-edge lights, floating panels, broad architectural neon, and tint-led phase color are prohibited. Amber remains localized to visible windows, entrances, and lamp falloff. Crimson is sparse and threat-specific. Every retained or added object supports building identity, navigation, surveillance, line-of-sight, interaction, safehouse/objective readability, or required civic atmosphere. The outside-bounds perimeter backdrop is unreachable, background-only, non-colliding, and excluded from playable-building density metrics.

### Internal milestones and proof gates

1. Commit and read back the documentation-only reference-native authority package with separate requester authorization. After raw-greybox acceptance, separately close `OPEN-LAYOUT-007`, freeze the exact seed/camera/probes/raw hash across authority surfaces, and obtain explicit authorization for that closure commit before source-facade production.
2. Add red-first contracts for the v6 recipe/runtime identity and one generated plan manifest. Version section 6 literally: bounds, street polygons, parcel footprints/heights, dynamic gate, relocated anchors, clearance discs, stable loop IDs/names, camera boxes, and 22 static plus four dynamic probes. The validator fails on any seed drift, footprint/route or fixture/disc intersection, wrong probe verdict, broken gate/bypass/reset behavior, or disconnected required anchor.
3. Completed: preserve v4/v5 and the rejected first v6 raw frame as recoverable evidence; pass the amended seed-only projection preflight; produce one material-free raw rerender with mission placeholders, reference/candidate, route overlay, per-identity masks, source/proportion feasibility, and a hidden answer key; freeze the requester-approved amended result, exact camera, and replacement probes.
4. After the amended layout-closure/source-hero authority commit, rebuild the source-derived v6 scene in four ordered phases. Phase 0 audits the donor and rejected v6 scene read-only, repairs images only inside a new derivative, saves/reopens it, and proves zero missing external images. Phase A replaces the camera-visible envelope layer with real modeled Neo Tokyo 2 assemblies on all 27 parcels and proves per-parcel collection/object/material/transform/bounds/hash provenance plus source-only silhouette IoU `>=0.85`; it also builds the HIDZU tower/gate identity as dimensional mounted geometry and passes the frozen-camera glyph preview. Phase B adds bounded, provenance-recorded life-street/junction clutter and varied source roof/storefront detail without intersecting routes, anchors, or staging wedges. Phase C adds only restrained depth fog, physically motivated practical falloff, the established AgX/Filmic wet-blue-black grade, and subtle neutral grain after geometry/material/light/legibility pass. Preserve detailed warm/cold windows, readable labels, controlled cool-white/cyan fixtures, sparse red warnings, one dominant HIDZU tower, real restricted-area treatment, readable cool shadow fill, continuous wet-ground response under every emitter, the dark sneak mouth, the transit group, and Needle's gate association.
5. Render one Cycles Metal blue-hour hero at `1440×900`, 45° azimuth, 30° elevation, target `{21.177121,17.126983,1.621967}`, and orthographic scale `25.687085`. Present `reference | candidate | delta notes` plus a verified 200% road/reflection crop and two-sided visual metrics. Stop for requester hero approval.
6. After hero approval, render aligned `6400×3600` dusk, blue-hour, and curfew people-free masters from the frozen v6 camera/geometry. Rebuild all desktop/mobile backgrounds and independent per-identity foreground slices/depth anchors. Rebaseline collision, placement metadata, state hashes, registration tests, visual masks, and regression frames.
7. Remove the random ambient allocation. Author exactly two seated plus one standing transit passenger, two seated café patrons, the public restricted-area guard, and Needle in explicit nonblocking runtime slots; remove the service-entrance guard. Make `blend.public_queue` reflect actual group/schedule state and keep `blend.delivery_activity` observably unavailable. Do not add detection, trespass, civilian reaction/pathfinding, or interception behavior.
8. Publish the complete schema-v2 state/profile matrix through staging and atomic replacement. Retain page-stable profile selection, typed phase resolution, prefetch, generation-token stale rejection, 750 ms crossfade, observable failure, and texture/GameObject disposal. Measure and obtain approval for new transfer and residency budgets; the v4 ceilings are not acceptance limits.
9. Inspect normal play, Observation and Suspicious/Pursuit presentation hooks, overview, mobile, both profiles, three occlusion sites, actor feet, zoom sweep, direct time jumps/rewinds, failure paths, and the fixed viewports. Present the live game side by side with the exact references and obtain requester acceptance before closeout or implementation commit.

### Content requirements

Produce the v6 recipe/runtime identities; exact plan manifest; dimensioned plan; parcel/anchor overlay; route/anchor/dynamic-gate report; 22-static/four-dynamic probe report; camera-box report; source/proportion feasibility; occlusion-aware per-identity masks/report; raw and annotated mission-legibility greybox evidence; per-parcel KB3D collection/object/material/transform/bounds/complexity/hash provenance and saved-scene readback for all 27 source assemblies; immutable-donor plus save/reopen zero-missing-image report; zero-camera-visible-envelope evidence; source-only silhouette-IoU report; bounded clutter inventory with source/purpose/clearance evidence; independent cutout/depth metadata; perimeter-backdrop declaration; checkpoint, transit, café, terminal, hide, and Needle slot manifests; facade/window/sign/light inventory; dimensional HIDZU tower/gate component inventory plus readable-glyph preview; one dominant HIDZU destination; restricted-area visual set; palette/value, wet-material, depth-fog, practical-falloff, grade, and grain guides; facade-detail and road crops; component-removal fingerprints; three aligned people-free environment exports; schema-v2 state/profile manifest; measured transfer/decode/residency report; and fixed live evidence. T5 may add small non-emissive district-wayfinding fixtures as part of the physical clutter layer, while authored route-state content remains GET-214-owned. T5 adds no cyan wayfinding or synthetic facade/building replacement.

### World/UI/audio/George feedback

World feedback prioritizes current objective/action, actor, observation/threat, traversal/entrance, building identity, then ambience. Cold shadow fill preserves facade and road detail. Each practical/emissive source has a credible local ground response. Camera coverage uses truthful restrained geometry and never paints buildings broadly cyan; its LED, IR glint, pavement response, and Observation geometry agree. Needle is neutral amber/warm-white while patrolling and crimson only while verifying or pursuing. The barrier, warning, sign, guard, and drone communicate the real direct-route boundary; opening/closed visuals must agree with collision. T9 owns HUD implementation; GET-208 owns detection, trespass, pursuit, and broader security/civilian behavior. George's canonical cyan-core orb remains a separate private live layer and is never baked into the city.

### Failure and recovery

The plan gate fails if implementation drifts from the exact amended seed without a canonical reason, projected roof/staging preflight fails, a footprint/fixture intersects a route or anchor clearance, any replacement probe fails, the barrier does not block/open/reset correctly, the bypass is disconnected, any required anchor is unreachable, a camera box misses, fewer than ten identities contribute `0.5%` each, one identity exceeds `15%`, rendered staging masks miss their floors, destination/obstacle/blend/hack/sneak is not pointable, source/proportion feasibility fails, or the requester rejects the frame. The source-hero gate fails if any of the 27 parcels lacks visible retained Neo Tokyo 2 geometry or complete saved-scene provenance, any camera-visible parcel envelope/generated facade supplies the apparent architecture, the source-only silhouette IoU falls below `0.85`, a deviation is not modeled relief, clutter lacks source/purpose/clearance evidence, any historical donor master changes, the reopened derivative has a missing external image, HIDZU identity is a plane/decal/post-render substitute, a required glyph is mirrored/unreadable, or facade detail still reads as painted rectangles at `200%`. The hero/live gate also fails on reference-adjacent rather than reference-native composition, decorative-only gate behavior, dark void roads, unreadable windows/facades, weak identity, random NPC placement, false blend eligibility, warm beige/ochre grade, dry/patchy roads, missing reflections, generic neon, synthetic architecture, crushed curfew blacks, photographic finish that conceals missing geometry, or background mass intruding into playable space. Missing initial art fails visibly. A failed transition keeps the current complete state; a failed render/publisher leaves the public root intact. Correct the responsible stage instead of weakening plan, route, probe, provenance, hash, or placement tests.

### Explicit exclusions

- No synthetic replacement of Neo Tokyo geometry or per-building collage.
- No mutation of the accepted GET-204 recipe or recoverable v3/v4 sources.
- No inheritance of the `58×44` grid, four-block invariant, old routes/anchors/camera, v5 density band, or recovered 24-point fixture as current acceptance.
- No facade/material/light work before the approved layout closure is committed/read back; no state/cutout/runtime publication before hero approval.
- No gameplay detection, trespass, network transitions, dialogue, facts, or objective logic.
...
- No clutter added solely to make the city look busy and no reachable false-building backdrop.
- No camera-visible procedural parcel envelope, emissive-decal facade, painted window grid, generated facade, or provenance-only claim in place of visible modeled Neo Tokyo 2 architecture.
- No mutation of a historical donor master, deleted temporary-cache dependency, session-only texture remap presented as durable, unresolved image after save/reopen, plane/decal HIDZU sign, or mirrored required glyph.
- No merged continuous-wall cutout, baked crowd/guard, random or wandering ambient civilians, capacity-exceeding/overlapping figure slots, café-derived generic cover, active blend eligibility without its group, or silent service-guard restoration.

### Dependencies and OPEN blockers

Depends on the recoverable GET-204 named-source baseline and the separately committed reference-native authority package. `OPEN-LAYOUT-007` is resolved by the approved exact seed/camera/probes/raw baseline; the real-geometry corrective build begins only after the amended layout-closure/source-hero authority commit is authorized, committed, and read back. `OPEN-PERF-001` owns the new measured publication budget. `OPEN-CIV-001` retains delivery/broader behavior tuning without reopening the T5 minimum; `OPEN-SEC-001` remains GET-208 tuning; the T5 gate collision is real while detection/trespass behavior stays outside this ticket. `OPEN-ART-005` remains actor-light work. T5 blocks downstream final visual integration until live acceptance. Raw Neo Tokyo 2 source remains outside Git.

### Canonical decisions/spec sections

Implements `GDR-SET-003`, `GDR-SET-005`, `GDR-SOC-001`, `GDR-SOC-003`, `GDR-SUR-008`, `GDR-SUR-011`, `GDR-SUR-012`, `GDR-SUR-013`, `GDR-CIV-002`, `GDR-CIV-003`, `GDR-ART-001`, `GDR-ART-007`, `GDR-ART-012` through `GDR-ART-016`, `GDR-ART-019`, `GDR-REM-011`, `GDR-SUP-001`, `GDR-SUP-004`, and the outdoor boundary in `GDR-PROD-004`. Canonical detail is in [[20 Setting & Worldbuilding]], [[30 Art Direction (MVP)]], [[32 GET-205 Reference-Native Layout Contract]], [[31 GET-204 Visual Rebuild Quality Contract]] as historical source evidence, [[42 Surveillance, Security & Civilian Behavior]], [[80 Day-Night Cycle]], [[35 Narrative Alignment]], [[47 Social Feed]], [[03 Lore/Plot Bible]] sections 3, 8, 11, and 12, and [[04 Engineering/Roadmap]] Gate 5.

### Human-play acceptance

The first requester gate is complete: the dimensioned v6 plan and amended mission-staged raw `1440×900` frame were approved on 2026-08-13 after the blind destination/obstacle/blend/hack/sneak read. The accepted raw frame is SHA-256 `ddf5a224da0eec2fe8af5b1c0bf2f7ca8f5e618080ef1d63b4a18c4c72aa7369`; its exact camera and all 22 static/four dynamic probes are frozen. The envelope-led dressed rounds are rejected. The second gate is one real-source blue-hour hero/reference/delta with per-parcel scene provenance, zero visible parcel envelopes, source-only silhouette IoU `>=0.85`, a `200%` facade-detail crop, a `200%` road crop, bounded clutter evidence, and rerun ART-016/identity/mission-legibility masks. The final gate covers live clean-world/current-HUD frames, aligned states, complete-neighborhood overview, both profiles, occlusion, actor feet, and the accepted v6 zoom sweep at `1280×720`, `1440×900`, `1920×1080`, and mobile `390×844`. It must prove the real checkpoint, bypass, relocated anchors, independent depth slices, exact mission groups, truthful blend availability, one guard plus Needle, wet/noir quality, state loading/transitions, disposal, and measured residency. Validators do not substitute for requester judgment.

### Documentation and validation obligations

Atomically update the Decision Register, Review Queue, new layout contract, Art Direction, this complete Linear source, GET-204 historical boundary, movement/content contracts, Roadmap, MVP Readiness, Architecture, implementation plan, progress note, and live Linear description before production work. Add red-first tests for v6 identity/recovery, exact plan generation, route/anchor/fixture clearance, 22 static and four dynamic probes, barrier state/reset, camera boxes, identity contributions, mission-slot ownership, per-parcel source collection/object/material/transform/bounds/hash provenance, immutable donor hashes, save/reopen zero-missing-image durability, dimensional HIDZU component ownership and glyph parity, zero visible proxy envelopes, source-only silhouette IoU, bounded clutter purpose/clearance, independent identity slices, authored population/security cuts, manifest/registration regeneration, and staging rollback. Retain every ART-016, window, identity, mission, shadow, road, amber, cyan, luminance-spread, and `p05` gate for the hero/states. Rebaseline every invalidated state, cutout, collision, placement, camera, payload, and regression artifact deliberately. After visual approval run relevant validators, lint, build, Jest, coverage above 80%, and the guided Level 0 agent playtest. Documentation and accepted implementation commits require separate explicit authorization; GET-205 remains non-terminal until requester verification of the committed build.
...
The prior characters were tiny or stiff against monumental buildings and drifted into fantasy-operative styling. Actor sheets, portraits, cover-select, dialogue identity, and world scale also came from inconsistent sources. The first T6 pass repaired the human cast but replaced George's long-developed orb identity with a generated human bust because his form had been left underspecified. T6 preserves the grounded cast work while restoring George's exact canonical orb identity and one manifest contract for every presentation asset.
...
- George's visible identity is approved by `GDR-GEO-005`: a dark circular orb with cyan concentric core, four axial ticks, central point, and angular/diamond framing, recovered from `GeorgeOrbLogo` at commit `49a4da7`. `OPEN-NAR-009` may still resolve origin, hardware, or channel exposure, but may not alter that visible form. Naila/Brant biography cues, entry-flow ownership, and shipping budgets remain explicit OPEN items. Actor proportion is no longer open; `GDR-ART-014` owns it. `OPEN-PERF-001` has no numeric baseline, so T6 may record measurements but cannot pass shipping-performance acceptance.

### Complete player flow

New Game presents the four authored covers, one playable and three visibly disabled; confirming the playable cover persists its appearance ID. In the safehouse and district, the protagonist's idle, facing, movement, and interactions align with their ground anchor. Lira, Naila, and Brant appear as the same identities used in dialogue/debrief presentation. Civilians visually support delivery activity, queues, waiting, and service roles; security reads as institutional verification staff rather than a combat class. At curfew and during surveillance transitions, actor value and silhouettes remain legible. George appears as the same private cyan-core orb near the protagonist and in his HUD identity; he never becomes a physical party member or human face/bust/body.
...
6. Preserve the twelve matching `256×256` identity portraits and Takahiro broadcast portrait; replace the rejected George human-bust presentation with a separately registered transparent canonical orb asset derived faithfully from `GeorgeOrbLogo` at `49a4da7`. Preserve its silhouette/geometry while allowing only material, motion, and state-intensity refinement. Validate provenance/hash/bytes and ensure no fallback can resolve to the bust.
...
Maintain actor/portrait manifests, stable preset/actor IDs, source references, asset-generation or painting recipes, frame normalization rules, direction/state/frame naming, anchors, scales, occupancy bounds, depth policy, neutral fallback policy, path safety, SHA-256, compressed/decoded byte metrics, and provenance. Identity portraits are `256×256`, contain one human identity with face/shoulders inside the central 80% safe area, and bake no localized text. George is not part of that portrait rule: his source record must name the recovered orb implementation and retain a comparison to it. Clothing must reflect expatriates, contacts, service/public civilians, and corporate security without weapons, fantasy armor, package colors, or unexplained military competence.

### World/UI/audio/George feedback

Facing, foot contact, locomotion, and interaction must be readable against the accepted GET-205 rebake without labels or x-ray effects. Cover-select and Character screen show the selected appearance once T9's shell is integrated. Dialogue/debrief identity matches the world actor. Footsteps/interactions emit semantic hooks for T10 audio but do not drive state. George's T6 base art is the recovered private cyan-core orb, visually distinct from every physical actor. Proof places it near the protagonist's upper-right at `28–36` screen pixels and in the George lane with identical core geometry; T9 owns final states, placement, prompts, and suppression. It never owns collision/occlusion/depth, implies that other characters see him, or displays a human face/bust/body.

### Failure and recovery

Production acceptance fails for missing matrices, anchor drift, sliding feet, mirrored/wrong direction, roof placement, scale mismatch, silhouette ambiguity, fantasy styling, portrait mismatch, unreadable curfew values, stale appearance after Restart Attempt/New Game, any George human face/bust/body, or a materially altered orb silhouette. Correct the asset/manifest/integration; do not move gameplay anchors or add labels. Restart Attempt restores authoritative actor state and keeps the selected protagonist identity without preserving post-departure dialogue, pursuit, or animation state.
...
- No invented Lira/Naila/Brant biography or George hardware before its OPEN decision. Hardware uncertainty cannot be used to replace, redesign, or postpone the approved orb form.
- No George human portrait, holographic head, bust, body, alternate logo, or fallback to the rejected GET-206 presentation.

### Dependencies and OPEN blockers

Depends on requester acceptance of the T5 blue-black wet rebake; consumes T3 anchors and later T7A/T9 identity state/surfaces. The grounded human actor batch remains reusable; its committed `0.96` scale is not accepted, and the rejected George bust does not remain. Blocks T9/T10/T10B final presentation. T6 owns stable actor origins/foot anchors, the one shared scalar derived under `GDR-ART-014`, and canonical George orb art; T10B alone owns light-region sampling/tint. High acceptance blockers are `OPEN-NAR-009`, `OPEN-NAR-010`, `OPEN-NAR-011`, `OPEN-UI-002`, and `OPEN-PERF-001`, but `OPEN-NAR-009` is narrowed to origin/hardware/channel exposure and cannot reopen `GDR-GEO-005`. Their recommendations may be trialed provisionally through replaceable manifests/assets and cannot be called accepted while open. `OPEN-UI-002` must preserve the Roadmap split: T6 owns appearance/George assets; T9 owns shell/layout.

### Canonical decisions/spec sections

Implements GDR-PC-002, GDR-GEO-001, GDR-GEO-005, GDR-ART-001, GDR-ART-005, GDR-ART-014, and GDR-REM-011. Canonical detail is in [[48 Actors & Portraits]], [[30 Art Direction (MVP)]] actor rules, [[31 GET-204 Visual Rebuild Quality Contract]], [[40 George (AI Companion)]], [[92 Character, Covers, Abilities & Research]] appearance contract, [[04 Engineering/Architecture]] Art/Actors contract, and [[04 Engineering/Roadmap]] Gate 6.

### Human-play acceptance

For the current T6 gate, select every protagonist preset through the neutral appearance seam across fresh runs and verify it in the safehouse world; final Character/dialogue/Restart Attempt/debrief persistence remains deferred to T7A/T9/T10. Meet Lira, Naila, and Brant and distinguish their world/portrait identities without labels. Inspect both reusable security and all three civilian visual archetypes without treating them as authoritative placed/scheduled actors. Inspect all twelve portraits and Takahiro. At 1280×720, 1440×900, and 1920×1080, compare the accepted GET-205 rebake at normal and overview framings across dusk, blue hour, and curfew for pixel-derived foot stability within two pixels, correct facing/state, no roof placement, and readable silhouettes. The `1440×900` normal frame must compare directly with the locked blend, measure approximately `68–80 px` of protagonist alpha body excluding shadow/ring/George, show all human roles at one base scale, and preserve the blend's door/sidewalk/street/building relationship through the full zoom sweep. In the same live frames, compare George side by side with the recovered `GeorgeOrbLogo`: the dark circular body, cyan concentric core, four axial ticks, central point, and angular/diamond framing must remain immediately recognizable in both world and HUD forms, with no human face/bust/body. Missing/corrupt assets must produce an observable neutral diagnostic and fail the production gate; fallback never counts as the production matrix or orb. GET-204, rejected GET-205 grade, greybox, oversized-actor captures, or human-bust captures do not satisfy the gate.
...
Conversation is gameplay. The player sees the exact line they will say, the exact met/not-met gate verdict before committing, the identical reasons afterward, and the declared worse path on nonterminal failure. General facts stay binary while the dedicated Cold Iron evidence chain creates one consequential street-to-document progression. George feels present through the same recovered cyan-core orb beside the protagonist and in his HUD lane, explains every information limit, and remains honest, bounded, and non-agentic. A slim four-lane dock keeps George and current task separate while the dossier/minimap reveal only what the protagonist knows. Recurring city screens also remain trustworthy interfaces: transit departures/time, verification procedure/verdict, and Hidzu Corporation sector advisories each have one stable job.

### Starting state

- T3 provides pause/focus, clock, layout anchors, interaction, discovered-world state, overlay ownership, and persistence.
- T7A provides identity/ability/gate/condition payloads; T8 provides network/context state and verified risk data; T6 provides portrait art and the canonical recovered George orb asset defined by `GDR-GEO-005`.
...
6. George offers only authored prompts valid for current context/verified ledgers. His private floating cyan-core orb and HUD-lane orb stay synchronized in geometry and state intensity; insufficient evidence always yields an honest reason rather than semantic silence. Before departure he reads the real `OperationAttemptBaseline` inputs/restoration meaning.
7. Hidzu Corporation feed/screens provide read-only propaganda, notices, curated civic sentiment, suppression, or transit context without posting/messaging simulation. The transit shelter shows departures and civic time; the verification lane shows procedure, exact verdict, and manual review; one advisory surface shows sector-scoped notices in a two-line normal-zoom layout. No screen changes jobs or leaks undiscovered facts.
...
`GeorgePrompt` is authored for allowed context, required facts/mission state, exclusions, question/response keys, explicit unavailable-reason keys, and `effect: none`. George cannot mutate the world, conceal rules through silence, or carry a deletion/freedom want in Level 0. His presentation state may change orb motion, pulse, cyan intensity, ticks, and framing emphasis, but never its canonical silhouette or into a human face/bust/body. The HUD owns presentation only; domain state remains in its system. George and current-task lanes remain separate. All overlays/dialogue own a named pause token and release it deterministically.

World-screen content uses three typed presentation roles rather than a generic carousel. `transit` consumes schedule/civic-clock state, `verification` consumes the lane's procedural and exact verdict state, and `sector-advisory` consumes only eligible authored `GDR-SOC-002` events. Each role has stable IDs, two-language content, knowledge filters, explicit empty/error states, and a fixed information budget; the advisory supports two readable lines at normal zoom. The provisional Japanese slogan and George's contextual translation remain governed by `OPEN-NAR-014` and cannot be frozen by implementation.
...
5. Implement world/HUD synchronization for the canonical George orb, contextual prompts with explicit insufficiency behavior, threshold one-shot lines, and departure readback. Verify the recovered orb grammar remains recognizable at every state and viewport.
6. Implement the four-lane 16–18% dock plus Character, dossier, dialogue, feed, and related overlays across all target viewports. Provide the three typed world-screen content/read-model contracts and prove that transit, verification, and sector-advisory surfaces cannot exchange roles or bypass knowledge filtering.
...
Provide reusable nodes/choice/effect/gate-verdict schemas; English/Ukrainian localization catalogs; portrait/name/history treatment; binary fact catalog/provenance labels; the four-state Cold Iron catalog and five-minute copy action; dossier sections; discovery/precision rules; George prompt/limit/readback/threshold lines; capture/deadline/resource failure models; current-beat models; social-feed cards/screens; typed transit/verification/sector-advisory display schemas; two-line advisory layout; overlay empty/error states; and semantic HUD tokens. T10/T10A author final mission/city lines, but T9/T9A provide representative samples for every effect, lock, fail-forward, cause-specific failure path, and display role. Accessibility content includes non-color risk labels, keyboard focus/order, scalable text, subtitles/captions, reduced motion/flash, and volume entry points once approved. `未来は今日つくられる` and George's contextual meaning “the future is made today” may exist only as replaceable `OPEN-NAR-014` candidate content until cultural/localization review accepts exact copy and placement.

### World/UI/audio/George feedback

The world stays visible behind anchored overlays. Amber denotes objectives/time/curfew, crimson only Pursuit/immediate danger, cyan connected Hidzu Corporation technology, and muted teal/bone neutral knowledge. Matte ink/angular surfaces replace glass blur and broad glow. Every choice/fact/objective/state change gets concise text and restrained semantic audio without covering the city. Transit, verification, and sector-advisory displays use distinct framing/content hierarchy and preserve their jobs across schedule/network changes; the advisory's two lines remain readable at normal zoom. George's world and lane forms use the same recovered cyan-core orb; state changes refine pulse/motion/intensity without replacing its silhouette. He comments only on verified facts/current state, explicitly names why useful information is unavailable, and cannot contradict the dossier, reveal unknown cameras, prescribe a hidden safe route, act as a generic chat box, ask to be deleted, or appear as a human face/bust/body.

### Failure and recovery

Missing localization or invalid node/effect/display data fails validation and cannot silently fall back to different semantics. A display with missing eligible data uses its own truthful empty/error state; it never borrows another role or fabricates a notice. A failed deterministic choice applies its authored worse path once and remains in history; only the final failed capture escape may terminate the attempt. George always explains an unavailable-information boundary and never encodes hidden meaning in absence. Capture reads only real surveillance-ledger evidence and preserves disconnected unseen gaps; deadline lists unfinished requirements; Paranoia and time consequences stay factual. Overlay errors preserve world state/pause ownership, then offer safe close/Restart Attempt. `OperationAttemptBaseline` restores committed domain state, while transient UI never mutates outcomes. Safehouse controls and George planning prompts show the approved or explicitly provisional `OPEN-SAFE-001` unavailable reason instead of creating a UI-only escape from active surveillance.
...
- No George movement, hacking, interaction, surveillance change, invented fact, undiscovered reveal, automatic choice, guaranteed uncertain prediction, hidden-information silence, deletion/freedom arc, human face/bust/body, or alternate-logo substitution.
...
- No generic world-screen carousel, interchangeable transit/verification/advisory surface, decorative ad rotation, undiscovered-fact leak, or provisional Japanese copy promoted to final without resolving `OPEN-NAR-014`.

### Dependencies and OPEN blockers

Depends on validated committed T3 and T7A foundations and consumes T6/T8 state. T6 must first deliver the requester-accepted canonical orb asset against the accepted T5 rebake. T9 blocks T9A and T10; T9A depends on T8A plus the GET-179 modernization milestone and blocks T10A. Critical acceptance blockers are `OPEN-NAR-004`, `OPEN-NAR-005`, `OPEN-NAR-007`, `OPEN-NAR-008`, `OPEN-NAR-015`, `OPEN-ABL-001`, and `OPEN-SAFE-001`. High acceptance blockers are `OPEN-NAR-006`, `OPEN-NAR-009` through `OPEN-NAR-011`, `OPEN-NAR-014`, `OPEN-UI-001` through `OPEN-UI-003`, `OPEN-LOC-001`, and `OPEN-ACC-001`. `OPEN-NAR-009` cannot reopen the visible form fixed by `GDR-GEO-005`; `OPEN-NAR-002` and `OPEN-PAR-002` are resolved. Recorded recommendations remain reversible/non-final while open.

### Canonical decisions/spec sections

Implements GDR-MIS-003 through GDR-MIS-008, GDR-PAR-003, GDR-PAR-007, GDR-TIME-002, GDR-OBS-002, GDR-STL-002, GDR-DLG-001 through GDR-DLG-003, GDR-RPG-007, GDR-FACT-001, GDR-FACT-002, GDR-GEO-001 through GDR-GEO-005, GDR-SAFE-001, GDR-FAIL-001, GDR-UI-001 through GDR-UI-003, GDR-SOC-001, GDR-SOC-003, GDR-REM-009, GDR-REM-010, and GDR-REM-012.
...
- Ask every available/unavailable George prompt in HUD, Observation, safehouse, and dossier; prove verified-only answers, explicit limit reasons, no semantic silence/personal arc, departure readback, zero world mutation, and one recognizable recovered orb identity across world and HUD states with no human substitute.
...
- Inspect the transit, verification, and sector-advisory surfaces at normal zoom in both languages; prove stable roles, two-line advisory readability, exact verification verdict parity, schedule updates, knowledge filtering, and truthful empty/error states without a generic carousel.
...
5. Hear George read the real departure time, contacts, the Paranoia tier, held abilities, and restoration meaning; confirm `OperationAttemptBaseline`. Choose Transit Road/Market Ring/Outer Space timing and adapt as the four clock moments change the street. At 18:45, the transit shelter/public queue visibly supports ordinary blending within its declared seated and standing capacity; after 21:30, the same place winds down and becomes inactive at curfew.
6. Read subtle camera warnings, use Observation for exact discovered coverage, remain harmless in ordinary public view, avoid or trigger observed rule breaks, use the single camera group once, and respond to Suspicious/Pursuit/Needle/interception as play produces. The pedestrian verification lane explains its commitment through queue rails, floor arrows, and an eye-height lit panel. Needle remains amber/warm-white while neutrally patrolling and turns crimson only while verifying or pursuing.
...
Ordinary public visibility is harmless. Surveillance concern requires paired visibility and approved rule-break evidence; normal geometry/occlusion creates blind spots; Clear resets recognition. The one camera group's terminal history persists until Restart Attempt. Camera LED, IR glint, wet-pavement cue, and Observation geometry resolve from the same device state. The verification lane is pedestrian and its premature-exit consequence remains exactly the previewed incomplete-processing event. Needle's lamp follows its actual neutral/verification/Pursuit state. Every gated choice shows identical preview/result verdicts and each nonterminal failure advances through a declared worse path. Four clock boundaries are idempotent. Transit, verification, and sector-advisory displays keep distinct roles and never leak unknown facts. Public-queue population respects visible seated/standing capacity and remains runtime-owned. Actor tint is presentation-only.
...
3. Deliver T10A's four street-clock moments, named-route/grounding/threshold/civilian content, honest transit-shelter capacity, stable transit/verification/advisory screen content, and three spatial sound thresholds with bilingual parity. Prove the same camera at 18:45 and after 21:30 with runtime-owned population only.
...
Deliver the complete English/Ukrainian semantic script; contact biographies/portraits/anchors; mission/objective/binary-fact/Cold-Iron/gate/effect data; Transit Road/Market Ring/Outer Space schedule content; four idempotent clock moments; civilian/security/device/context placement; one camera group and Needle patrol; the pedestrian verification-lane procedure and exact verdict content; a transit shelter with declared seated/standing capacity; transit-departure/civic-time, verification, and two-line sector-advisory content; three one-function terminals; cache/medkit/manifest-recognition/five-minute-copy interactions; two grounding actions and three threshold lines; three spatial ambience locations; contextual onboarding; all cause-specific failure/debrief/completion variants; cover/ability/fact consequences; Miami continuation payload; public Hidzu Corporation feed/signage excerpts; and actor-light metadata. Every branch declares prerequisites, visible consequence, ledger write, localization, Restart Attempt behavior, and fallback. The provisional rooftop slogan/George translation remains replaceable under `OPEN-NAR-014` until approved.

### World/UI/audio/George feedback

The accepted continuous city, grounded actors, separate George/current-task lanes, overlays, and graphic surveillance-noir semantics remain coherent through all four clock moments. Objectives outrank ambient art; characters/entrances/contexts remain readable without permanent labels. Normal camera coverage reads through a truthful status LED, IR glint, and restrained wet-pavement cue while Observation shows the same discovered geometry exactly. The pedestrian verification lane is readable before commitment. Needle's neutral/active lamp state is truthful. Transit, verification, and advisory displays retain recognizable one-function identities. Audio includes the three threshold locations plus Needle and clock cues; no voice acting is required. Actors receive only subtle semantic amber/cyan foot-anchor tint. George teaches immediate context, explains unavailable information, reads departure truth, warns once at 40/70/90, and never solves the route or expresses a deletion arc.
...
- No vehicle-checkpoint substitute for the pedestrian lane, neutral red Needle lamp, screen-role carousel, baked population, occupancy beyond visible shelter capacity, synthetic rooftop garnish, or provisional Japanese copy treated as final.
- No reopening approved mechanic ownership inside the integration ticket; required changes return to the owning ticket/spec.

### Dependencies and OPEN blockers

Depends on validated committed T3–T9 deliverables. T10A additionally depends on T9A; T10B depends on T10A. Critical acceptance blockers are `OPEN-NAR-001`, `OPEN-NAR-004`, `OPEN-NAR-005`, `OPEN-NAR-007`, `OPEN-NAR-008`, `OPEN-NAR-015`, `OPEN-ABL-001`, `OPEN-ABL-002`, `OPEN-NAR-016`, `OPEN-PAR-001`, `OPEN-TIME-001`, `OPEN-CIV-001`, `OPEN-SEC-001`, and `OPEN-SAFE-001`. High acceptance blockers are `OPEN-NAR-003`, `OPEN-NAR-006`, `OPEN-NAR-010` through `OPEN-NAR-014`, `OPEN-AUD-001`, `OPEN-LOC-001`, `OPEN-ACC-001`, `OPEN-PERF-001`, and `OPEN-ART-005`. `OPEN-NAR-002`, `OPEN-PAR-002`, and `OPEN-LAYOUT-007` are resolved. Recorded recommendations remain explicitly provisional until accepted.

### Canonical decisions/spec sections

Implements the end-to-end current contract, especially GDR-PROD-001 through GDR-PROD-003, GDR-SET-001 through GDR-SET-007, GDR-PC-004, GDR-PC-005, GDR-CAMP-001, GDR-MIS-001 through GDR-MIS-010, GDR-RPG-007 through GDR-RPG-010, GDR-HLT-004, GDR-PC-006, GDR-PROD-005, GDR-INT-002, GDR-SUR-006 through GDR-SUR-012, GDR-SOC-001 through GDR-SOC-003, GDR-CIV-001 through GDR-CIV-002, GDR-PAR-005 through GDR-PAR-009, GDR-TIME-001, GDR-TIME-004, GDR-SAFE-001, GDR-INT-001, GDR-STL-003, GDR-ESC-001, GDR-ESC-002, GDR-DLG-001, GDR-DLG-003, GDR-FACT-001, GDR-FACT-002, GDR-GEO-002, GDR-GEO-004, GDR-FAIL-001, GDR-AUD-001, GDR-AUD-002, GDR-ART-011, GDR-ART-015, GDR-GOV-001, and GDR-GOV-006.
...
Walking normally through a public camera's view is safe. Concern starts only when a current observer sees a restricted-area breach, protected interaction, medkit removal, failed verification, or detected camera-feed change. Buildings create real blind spots. Normal play gives truthful LED, IR-glint, and wet-pavement warnings; Observation shows the exact same discovered coverage. The player can enter one clearly signed pedestrian verification lane, use one camera group once, read Needle's neutral versus verification state before it arrives, recover to Clear, and blend at a transit shelter whose visible capacity and operating hours are honest. Civilians react visibly without becoming secret informants.
...
1. Enter discovered public camera coverage, observe the device LED/IR glint and restrained wet-pavement cue, and remain `Clear` with no Paranoia.
...
4. Approach the pedestrian verification lane, read its eye-height instruction panel and floor/queue direction before entry, then prove a normal pass and a separate premature-exit incomplete-processing consequence.
5. Enter Suspicious/Pursuit, receive truthful last-known feedback, and hear Needle's hum/approach/verification warnings during its one patrol. Verify its warm-white/amber neutral lamp becomes crimson only during verification/Pursuit.
6. Break observation, evade through a credible context, return fully to `Clear`, and confirm recognition is empty.
7. Use the one camera group; let active looping expire and confirm `clean`/`traced` remains until Restart Attempt and prevents reuse.
8. At the transit shelter, prove blending eligibility depends on authored seated/standing capacity and schedule; observe civilians glance/step aside from visible events, then prove they never reveal hidden state or report the player.

### System rules and state transitions

`ObservationEvidence` remains raw geometry truth. `SurveillanceRuleBreakEvidence` exists only for `restricted-area-breach`, `protected-interaction`, `medkit-removal`, `failed-verification`, and `detected-camera-feed-change`, with observer/source/time/position. Concern and surveillance-origin Paranoia require both current valid visibility and valid rule-break/Pursuit evidence. Ordinary occlusion defines blind spots; no off-grid zone exists. Camera status LED, IR glint, restrained pavement reflection, and exact Observation geometry resolve from the same device state and coverage source. Camera-group history is `unused | active | clean | traced`; `active` may expire only to `clean`/`traced`, never `unused`. Full `Clear` resets recognition. Needle is the single player-facing verifier; neutral patrol uses warm-white/amber and only active verification/Pursuit uses crimson. The verification-lane commitment begins only after the player passes the readable pedestrian entry boundary; premature exit creates the exact previewed `Suspicious` incomplete-processing event and never instant capture. `blend.public_queue` enforces the transit shelter's declared seated and standing capacities plus schedule eligibility. Civilian reaction is presentation-only.
...
3. Bind device LED, IR glint, restrained wet-pavement warning, and exact Observation display to the same discovered geometry/occlusion resolver.
...
5. Author Needle's patrol/hum/approach/verification package, truthful neutral/crimson lamp transitions, and legitimate last-known behavior.
6. Implement the pedestrian verification-lane entry commitment, exact pass verdict, manual-review state, and premature-exit evidence using the T5-authored queue rails, floor arrows, and eye-height panel.
7. Implement full-Clear recognition reset, transit-shelter capacity/schedule eligibility, and visible-only civilian presentation.
8. Derive the capture evidence ledger needed by T9A without exposing a complete path.

### Content requirements

Author the five rule-break definitions and localized source labels; one camera-group/terminal mapping; device LED/IR/reflection/Observation metadata; solid-occluder cases; one Needle patrol plus hum/approach/verification cues and lamp-state mapping; one pedestrian verification-lane commitment boundary with instruction/pass/manual-review/incomplete-processing states; one transit-shelter blending context with exact seated and standing capacities plus schedule states; recognition-reset feedback; small civilian glance/movement cues; persistent surveillance ledger entries; and clean/trace/reuse-blocked strings. All content is bilingual and uses stable IDs.

### World/UI/audio/George feedback

Normal play communicates cameras through status LEDs, IR glints, wet-pavement reflections, and restrained sweep/focus effects; it does not paint exact cones everywhere. Observation may render exact discovered geometry. Needle has a distinctive hum, explicit approach/verification warnings, and a truthful warm-white/amber-to-crimson lamp transition. The pedestrian lane communicates commitment before entry through its physical direction and instruction display. The transit shelter shows whether blending capacity remains through visible runtime occupants rather than a hidden count. Civilians never become a network HUD. George may report verified source/last-known/recovery state and explicitly explain when a non-networked/undiscovered source limits him; he cannot infer hidden coverage.

### Failure and recovery

Suspicious/Pursuit remain recoverable under T8 rules; full Clear erases recognition but not persistent camera-group history. A missing/invalid rule-break record cannot escalate the network or add Paranoia. A detected feed change may create concern/trace but cannot be fabricated after the fact. Invalid/missing lane or shelter metadata disables that interaction with a truthful reason rather than inventing commitment or capacity. Needle's lamp may never announce a state the network does not hold. Capture records only real ledger evidence for T9A. Restart Attempt restores baseline camera/recognition/ledger state and resets post-departure use/events.
...
- No vehicle-checkpoint substitute, hidden verification commitment, instant capture for premature exit, red neutral Needle lamp, arbitrary “dark tile” blending, overcrowded shelter, or baked civilian used as capacity truth.
...
- Enter the pedestrian lane only after reading its direction/instruction; prove valid-cover passage and a separate premature-exit `Suspicious` result without capture.
- Compare Needle's neutral patrol and verification/Pursuit states; crimson must appear only during the latter and clear afterward.
- At 18:45, fill and partially fill the transit shelter within its exact seated/standing capacity; after 21:30 and at curfew, prove wind-down/inactivity without baked people or invisible capacity.
...
- No model identity claims from requested CLI flags alone; the tiered gate requires actual-worker runtime evidence, does not replace human visual/play acceptance, and adds no gameplay mechanic.
...
- T9 supplies dialogue/effect/fact/dossier/minimap/George/HUD/overlay infrastructure, the separate George/current-task lanes, and the synchronized canonical orb presentation from T6.
...
5. Ask George for known/unknown/non-networked information and receive verified content or an explicit truthful limit reason; watch the same world/HUD orb acknowledge the state without changing identity, and never infer from silence.
...
4. Add explicit George limit reasons, synchronized canonical-orb world/lane presentation, threshold/readback integration, and no-semantic-silence or identity-substitution validation.
...
Gate verdicts stay compact and attached to the choice; results visually preserve the same reasons. The four-lane HUD keeps George and current task distinct. George uses one concise live sentence per prompt/threshold/readback beat while the same recovered orb remains visible in world and lane; never an invented inner want or human substitute. Capture presentation reads as a restrained Hidzu Corporation incident report and partial map, not an omniscient replay. Cause-specific semantic cues may differ, but missing audio never changes state.
...
- No George hidden silence, free text, deletion/freedom arc, automation, invented knowledge, George/current-task merge, human face/bust/body, alternate-logo substitution, Bible epigraph decoration, or Observation vignettes.
...
- Ask George across known, undiscovered, non-networked, and unavailable contexts; verify explicit reasons, no hidden silence/arc, separate lanes, threshold history, exact departure readback, and synchronized recovered-orb identity at every state with no human substitute.
...
At 21:00, 21:30, 22:00, and 23:30 the street visibly and audibly tightens: announcements, shutters, crowds, lighting, and last-train cues make the deadline felt. The transit shelter is credibly populated at 18:45, visibly winds down after 21:30, and stops serving as a blending context at curfew; seated and standing occupancy never exceeds visible capacity. Transit, verification, and sector-advisory screens keep stable jobs while their content changes with legitimate time/network state. Locals/signage call the loops Transit Road, Market Ring, and Outer Space. The player can spend ten minutes at vending-machine coffee or a shrine for ten Paranoia relief once each, hears George warn once at 40/70/90, and can locate life behind three street thresholds.
...
2. Compare the same transit-shelter camera at 18:45 and after 21:30, then cross 21:00, 21:30, 22:00, and 23:30 through real-time play, Wait, pause/resume, and save/restore; each boundary changes the street exactly once and all people remain runtime-owned.
...
Clock boundaries are stable idempotent IDs at 21:00/21:30/22:00/23:30 and persist in `processedBoundaryIds`. Pause never advances time; explicit jumps process every crossed boundary once in order. Stable loop IDs remain unchanged; localized display keys yield Transit Road, Market Ring, and Outer Space. `blend.public_queue` uses an authored transit shelter with an exact visible seated count of two or three plus a separately declared standing capacity; runtime occupancy cannot exceed either envelope, claims no unavailable seat, is populated at 18:45, winds down after 21:30, and is inactive at curfew. The transit display remains departures/civic time, the verification display remains procedure/verdict/manual review, and the advisory remains eligible sector-scoped notices in a two-line normal-zoom layout. Each typed grounding action costs exactly ten world minutes, applies exactly −10 Paranoia, requires explicit confirmation/interaction, and has `usesPerAttempt: 1`. Difficult-escape relief is exactly −5 once. George threshold history is exactly 40/70/90 once per attempt. Dialogue grants no grounding relief.
...
3. Author civilian/schedule/shutter/signage/light/audio changes for each boundary using approved or reversible provisional values. Bind transit-shelter seated/standing occupancy and blending eligibility to those schedules without baking people into environment assets.
...
6. Author the transit-departure/civic-clock, verification procedure/verdict/manual-review, and two-line sector-advisory content streams without allowing surfaces to exchange roles or leak knowledge.
7. Spatialize the three authored ambient sources and validate falloff/priority/ducking.
8. Prove bilingual normal-control routes, same-camera 18:45/post-21:30 contrast, and pacing at all viewports.

### Content requirements

Provide loop display-name keys and signage/civilian/George usages; four boundary IDs with PA/shutter/crowd/light/last-train changes; a transit-shelter blending definition with exact seated/standing capacities and 18:45/post-21:30/curfew states; transit-departure/civic-clock, verification, and sector-advisory content keyed to legitimate state; two grounding anchors/interactions/confirmations/used states; three George threshold lines per language; one qualifying escape feedback; Transit Road restaurant, Market Ring workshop, and safehouse apartment ambience emitters; subtitle/caption labels; schedule states; and persistence/acceptance fixtures. The provisional Japanese slogan and George translation remain replaceable under `OPEN-NAR-014` until reviewed.

### World/UI/audio/George feedback

The city carries time through restrained PA announcements, shutters, truthful transit-shelter population change, practical-light changes, screen updates, and last-train chimes; the HUD remains truthful but secondary. Visible occupants preserve exact shelter capacity and are never flattened into the world plate. Transit, verification, and advisory displays retain distinct visual/content identities. Ambient sound leaks through believable thresholds rather than flat global loops. Grounding has a small human animation/text/audio beat, not a vignette or collectible. George says one concise authored threshold line, names routes where useful, may contextually translate only legitimately inspectable approved signage, and never supplies hidden navigation or relief himself.

### Failure and recovery

A boundary bundle is idempotent: duplicate frame/save/jump evaluation cannot replay effects/audio/state. Missing/invalid shelter-capacity or display-role data disables the affected context/surface with a truthful diagnostic; it cannot overfill, invent a seat, bake a substitute crowd, borrow another screen's content, or reveal an undiscovered fact. A used grounding action stays unavailable with a truthful reason until Restart Attempt; insufficient time or invalid interaction cannot apply partial effects. Deadline behavior remains T9A's unfinished-requirement report. Missing audio cannot block a boundary/resource transition and must fall back to text/visual cues. Restart Attempt restores exact processed/used/announced history from baseline.
...
- No baked people, invisible/over-capacity shelter occupancy, generic display carousel, screen-role swapping, decorative ad substitution, undiscovered-fact leak, or provisional Japanese slogan promoted to final.

### Dependencies and OPEN blockers

Depends on T9A and existing GET-210 city/content prerequisites; blocks T10B and final GET-139 acceptance. Relevant provisional blockers include `OPEN-TIME-001`, `OPEN-CIV-001`, `OPEN-LAYOUT-003`, `OPEN-NAR-014`, `OPEN-AUD-001`, `OPEN-LOC-001`, `OPEN-ACC-001`, and `OPEN-PERF-001`. `OPEN-LAYOUT-007` is resolved; approved exact layout/camera/probes, boundary times, route names, grounding values, difficult relief, and threshold counts cannot be changed by tuning.

### Canonical decisions/spec sections

Implements GDR-TIME-004, GDR-SET-007, GDR-AUD-002, GDR-SOC-002, GDR-SOC-003, GDR-CIV-002, GDR-ART-015, GDR-PROD-005, GDR-PAR-006, and GDR-PAR-007, with existing time/schedule/safehouse/audio decisions. Owns the reactive advisories (per OPEN-SOC-001), the sparse civic clock anchors, per-area tension identities, runtime population/shelter eligibility across the same-camera time contrast, and relief geography. Canonical detail is in [[80 Day-Night Cycle]], [[20 Setting & Worldbuilding]], [[47 Social Feed]], [[49 Audio]], [[60 Paranoia]], [[44 Safehouse, Save & Restart Attempt]], [[13 Level 0 Content and State Matrix]], [[04 Engineering/Architecture]] time/audio sections, and [[04 Engineering/Roadmap]] T10A.
...
- Capture the same transit-shelter camera at 18:45 and after 21:30, then at curfew; prove runtime-owned people, exact seated/standing capacity, visible wind-down, and blending eligibility/inactivity.
- Inspect all three recurring display roles at normal zoom in both languages; verify departures/time, verification procedure/verdict/manual review, and two-line eligible sector advisories never exchange surfaces or leak hidden facts.
