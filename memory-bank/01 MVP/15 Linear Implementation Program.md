...
The accepted GET-204 source established the mission-sized four-block envelope, but the live GET-205 v4 treatment still reads too dark, too open, and too weak in facade/window identity. Lighting corrected the color family but could not create the continuous street-wall density of the locked blend. T5 therefore performs a versioned rebuild: preserve the mission's executable roads, alleys, crossing, safehouse, anchors, and loops while repacking dead block-interior gaps into denser source-traceable massing. It then applies the approved wet blue-black Hidzu Corporation look and removes the current random ambient NPC allocation.

### Player promise

The four mission blocks read as one dense, inhabited Tokyo district rather than isolated buildings on a dark board. Continuous street walls, 12–16 distinct building identities, richer labels, detailed warm and cold windows, controlled colored fixtures, one dominant Hidzu Corporation landmark, a visual restricted entrance, and outside-bounds backdrop mass produce the locked blend's urban density without deleting a required route. The shared look remains wet blue-black surveillance noir: readable cool midtones, continuous rain sheen, crisp broken reflections, localized amber practicals, declared cyan technology/facade accents, and sparse threat-specific red. No random ambient civilians are shown, and blend contexts are honest about being unavailable until GET-208 restores authored population.
...
The player leaves the safehouse through the preserved threshold/court into a denser street canyon. The same public and service routes, contacts, terminals, hides, entrances, objective approach, crossing, and traversal loops remain reachable, but dead internal gaps no longer make the blocks read as isolated props. Facade labels, window patterns, cool service light, localized warm interiors, sparse red warnings, and one unmistakable large Hidzu Corporation building make blocks individually legible. The controlled approach reads as a restricted pedestrian area through barrier geometry, warning lights, signage, one static public guard, and Needle; T5 implements no trespass/detection behavior. The service entrance stays traversable and intentionally has no static guard in this ticket.

The environment remains people-free. Every current ambient civilian is removed from the live T5 presentation, while the protagonist, authored contacts, one public restricted-area guard, and Needle remain runtime-owned. `blend.public_queue` and `blend.delivery_activity` return an explicit unavailable state until GET-208 restores authored, capacity-honest groups and schedules. Dusk, blue hour, and curfew change windows, practicals, surveillance emphasis, and sparse threat color without moving geometry or hiding routes.
...
1. Commit and read back the documentation-only entry package with separate requester authorization. Record the v5 recipe/runtime identities, supersession boundary, preserved-topology schema, invalidated artifacts, population/security cuts, OPEN items, and proof gates before production changes.
...
3. Build only the v5 greybox from named Neo Tokyo 2 masses. Use 12–16 individual building identities, close dead internal gaps into continuous street walls, retain the required playable corridors, and add an unreachable perimeter backdrop. Render the fixed `1440×900` hero registration with no materials, facade dressing, lighting, or people. Present the locked blend, candidate, versioned building mask/delta, measured fraction band, and green route report; stop for requester massing approval.
...
7. Remove all ambient civilian presentations, retain exactly the public restricted-area guard and Needle, remove the service-entrance guard, and make both population-owned blend contexts observably unavailable. Do not add detection, trespass, schedule, or interception behavior.
...
Produce the v5 recipe and runtime identities; preserved-topology dataset; exact 24-point route fixture; route/anchor/probe report; versioned reference building mask and density report; greybox evidence; source/provenance inventory for every 12–16 building identity; independent cutout/depth metadata; perimeter-backdrop declaration; facade/window/sign/light inventory; one large Hidzu Corporation landmark; restricted-area visual set; palette/value and wet-material guides; component-removal fingerprints; three aligned state exports; schema-v2 state/profile manifest; measured transfer/decode/residency report; and fixed live evidence. Existing non-emissive route signage remains GET-214-owned. T5 adds no cyan wayfinding or synthetic facade/building replacement.
...
- No merged continuous-wall cutout, baked crowd/guard, random ambient civilians, active blend eligibility without population, or silent service-guard restoration.

### Dependencies and OPEN blockers

Depends on the accepted GET-204 source and the separately committed density-rebuild documentation gate. The requester-authored 24-point table is recovered with exact stable-v4 provenance. `OPEN-LAYOUT-006` blocks massing acceptance until all 14 Class A outcomes remain green and all 10 Class B points have fresh v5 results frozen with requester massing approval. `OPEN-ART-006` owns the provisional building-mask/fraction tolerance and must be resolved with accepted massing evidence. `OPEN-PERF-001` owns the new measured publication budget; the v4 payload ceilings are invalid. `OPEN-CIV-001` and `OPEN-SEC-001` remain GET-208 tuning items and are not resolved by T5's explicit population/security presentation cut. `OPEN-ART-005` remains actor-light work. T5 blocks downstream final visual integration until its live acceptance. Raw Neo Tokyo 2 source remains outside Git.

### Canonical decisions/spec sections

Implements `GDR-SET-003`, `GDR-SET-005`, `GDR-SOC-001`, `GDR-SOC-003`, `GDR-SUR-008`, `GDR-SUR-011`, `GDR-SUR-012`, `GDR-SUR-013`, `GDR-CIV-002`, `GDR-CIV-003`, `GDR-ART-001`, `GDR-ART-002`, `GDR-ART-003`, `GDR-ART-012`, `GDR-ART-013`, `GDR-ART-014`, `GDR-ART-015`, `GDR-ART-016`, `GDR-ART-017`, `GDR-REM-011`, `GDR-SUP-004`, and the outdoor boundary in `GDR-PROD-004`. Canonical detail is in [[20 Setting & Worldbuilding]], [[30 Art Direction (MVP)]], [[31 GET-204 Visual Rebuild Quality Contract]], [[42 Surveillance, Security & Civilian Behavior]], [[80 Day-Night Cycle]], [[35 Narrative Alignment]], [[47 Social Feed]], [[03 Lore/Plot Bible]] sections 3, 8, 11, and 12, and [[04 Engineering/Roadmap]] Gate 5.

### Human-play acceptance

The first requester gate is the people-free greybox at the fixed `1440×900` hero camera: locked blend, candidate, versioned building mask/delta, candidate/reference building-pixel fraction, green route/anchor report, identical outcomes for all 14 Class A probes, and fresh observed outcomes for all 10 Class B probes. The accepted greybox freezes those Class B expectations. Only after that approval may facade and lighting work begin. The second gate is the blue-hour hero/reference/delta comparison and 200% road crop. The final gate covers live clean-world/current-HUD frames, aligned three-state strip, overview, both profiles, three occlusion sites, actor-foot stability, and the `0.60→3.25` zoom sweep at `1280×720`, `1440×900`, `1920×1080`, and automatic mobile `390×844`. It must prove all invariant routes/anchors, 12–16 independent depth slices, one Hidzu landmark, the visual restricted entrance, zero ambient civilians, unavailable empty blend contexts, exactly one public static guard plus Needle, readable cold/warm windows, sparse red, continuous wet reflection, initial state, both prefetches/boundaries, direct jumps, rewinds, stale completion, failure retention, 750 ms crossfade, texture disposal, and measured one-/two-set residency. The requester must agree that the live scene materially matches the locked density and wet-noir references; validators do not substitute for that judgment.

### Documentation and validation obligations

Atomically update the Decision Register, Review Queue, Art Direction, this complete Linear source, GET-204 historical boundary, Surveillance/Civilian specification, Content Matrix, MVP Readiness, Architecture status boundary, implementation plan, progress note, and live Linear description before production. Add red-first tests for recipe/runtime supersession, the preserved-topology schema, route/anchor intersection, the 14 fixed Class A probes, fresh observations and later frozen expectations for the 10 Class B probes, 12–16 identity slices, population/security cuts, manifest/registration regeneration, and staging rollback. Extend the fixed-frame delta validator with a versioned building mask and two-sided density band against the locked blend; retain the versioned shadow, road, amber, cyan, luminance-spread, and `p05` gates for the later hero/states. Rebaseline every invalidated state, cutout, collision, placement, payload, and regression artifact deliberately. After visual approval run relevant asset validators, lint, build, Jest, coverage above 80%, and the guided Level 0 agent playtest. Both the documentation entry commit and accepted implementation commit require separate explicit authorization; GET-205 remains non-terminal until requester verification of the committed build.
