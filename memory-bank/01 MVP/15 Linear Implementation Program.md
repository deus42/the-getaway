...
The accepted GET-204 source established the mission-sized four-block envelope, but the live GET-205 v4 treatment still reads too dark, too open, and too weak in facade/window identity. Lighting corrected the color family but could not create the continuous street-wall density of the locked blend. The first v5 greybox then proved that coverage alone is insufficient: it passed topology and the density floor while rendering as two anonymous compounds with no readable mission. T5 therefore performs a versioned mission-staged rebuild: preserve the executable roads, alleys, crossing, safehouse, anchors, and loops; repack dead block-interior gaps into denser source-traceable parcels; and compose the destination, checkpoint, blend crowd, hackable fixtures, and sneaking route from their real anchors before applying the wet blue-black Hidzu Corporation look.

### Player promise

The four mission blocks read as one dense, inhabited, playable Tokyo district rather than isolated buildings on a dark board. One glance shows where to go, what blocks the direct approach, where to blend, what to hack, and where to sneak. Continuous street walls, 12–16 materially visible building identities, richer labels, detailed warm and cold windows, controlled colored fixtures, a dominant Hidzu Corporation destination at the public logistics entrance, its checkpoint, and outside-bounds backdrop mass produce the locked blend's urban density without deleting a required route. Random ambient allocation is removed, but purposeful public life remains: two seated plus one standing transit passenger, two seated café patrons, one public guard, and Needle, all separately rendered at declared mission slots. The shared look remains wet blue-black surveillance noir with readable cool midtones, continuous rain sheen, crisp broken reflections, localized amber practicals, declared cyan technology/facade accents, and sparse threat-specific red.

### Starting state

- GET-204's named-KitBash source and accepted recipe remain immutable and recoverable. `get205-four-block-baked-v3` and the wet-blue-black v4 source/runtime are also recoverable technical baselines. The requester rejected v4 on `2026-08-11` because its live frame is too dark, has weak window/facade detail, too much open interstitial space, too few distinct building identities, and randomly allocated NPCs. Its three-state renderer, transition work, and validators are salvage, not visual approval.
- GET-205 v5 uses a new recipe identity, `get205-dense-four-block-v5`, and a new runtime identity, `get205-dense-four-block-production-v2`. It never mutates or republishes the accepted GET-204 recipe or v4 identity in place.
- The first v5 coverage greybox is rejected but recoverable. It passed all route/anchor/probe checks and measured `0.90755946` of the reference building fraction, yet `v5.public.market` occupied `37.69%` and `v5.service.workshop` `23.20%` of the fixed hero while fourteen declared identities were materially hidden. Its recipe, scene, hashes, and reports are failure evidence, not a massing lock.
- Preserved topology is the versioned invariant dataset: two roads, three alleys, the crossing, safehouse threshold and court, three traversal loops, and every proof start, contact, entrance, terminal, hide, blend, objective, and drone-launch anchor with a clearance disc. Formerly walkable dead gaps inside the blocks are not preserved; they may become building footprint only when no route polygon or clearance disc is intersected.
- Reference authority is explicit: `kitbash-reference2-blend-concept-v1.png` owns four-block massing, density, source/composition relationship, and human/door/sidewalk/street/building proportions without becoming production geometry. `canvas-quality-target.png` owns wet blue-black material/light/reflection quality. `street-play-target.png` owns normal-camera intimacy, social staging, surveillance readability, and HUD/world balance but not actor size. `dense-city-target.png` remains an overview-density check only.
- The v5 rebuild invalidates all v4 state masters, cutouts, collision/placement pins, transfer ceilings, regression frames, and runtime identity expectations. Those artifacts are regenerated only after the appropriate visual gates.
...
- T5 may change versioned building footprints, materials, signage, lighting, civic/surveillance props, atmosphere, and flattened derivatives only through the declared gates. It may not intersect preserved routes/anchor clearances, alter device mechanics, or weaken route/probe verdicts to fit art.

### Complete player flow

The player leaves the safehouse through the preserved threshold/court into a denser street canyon. The same public and service routes, contacts, terminals, hides, entrances, objective approach, crossing, and traversal loops remain reachable, but dead internal gaps no longer make the blocks read as isolated props. The tallest Hidzu mass lands at `entrance.logistics.public`, right-of-centre in the fixed hero, so the destination is visible from the public approach. Its base stages the restricted pedestrian checkpoint and `camera.public_gate`; barrier/guard-post geometry blocks the restricted spur visually without intersecting the protected public road or entrance clearance. Needle's launch/patrol volume remains visibly associated with that gate. The service entrance stays traversable and intentionally has no static guard in this ticket.

The transit shelter, bench, timetable fixture, exactly two seated passengers, and one standing passenger stage `blend.public_queue`; that context is eligible only while its actual group and schedule are active. A street-level café near the Transit Road restaurant stages exactly two seated patrons at declared furniture without creating generic cover. Street-facing fixtures mark all three terminal anchors, and tight recesses mark all three hide anchors so hack and sneak options exist in the scene before facade glow or HUD explanation. `blend.delivery_activity` remains explicitly unavailable until GET-208 authors its population. All people remain runtime-owned and absent from environment plates. Dusk, blue hour, and curfew change windows, practicals, surveillance emphasis, population schedule, and sparse threat color without moving geometry or hiding routes.

### System rules and state transitions

The T5 lifecycle is `COMMITTED_TECHNICAL_BASELINE → DENSITY_REBUILD_DOCUMENTATION_COMMIT → MASSING_GATE → HERO_FRAME_LOCK → THREE_STATE_EXPORT → CUTOUT_PROFILE_MANIFEST_REGEN → LIVE_SIDE_BY_SIDE_ACCEPTANCE → ACCEPTANCE_COMMIT → COMMITTED_BUILD_VERIFICATION`.

The new recipe may change building footprints only inside the preserved-topology constraints. A route-graph validator rejects any footprint that intersects a declared route polygon or anchor clearance disc and executes the recovered requester-authored 24-point collision table. Its 14 Class A geometry/bounds probes (`1–12`, `16`, `17`) must keep identical outcomes; its 10 Class B actor/interstitial/guard-dependent probes (`13–15`, `18–24`) retain their v4 observations as provenance and receive new expected outcomes only from the requester-approved v5 greybox. The massing gate compares candidate/reference building-pixel fraction in the provisional `0.95–1.05` mid-band and renders occlusion-aware per-identity masks at `720×450`: at least ten identities must each contribute at least `0.005` of the full fixed hero, and none may exceed `0.15`. The raw `1440×900` frame must answer the five mission-legibility questions without relying on diagnostic labels. No facade identity, authored material, lighting, state bake, cutout, or runtime publication starts until the requester approves the corrected greybox.

After massing approval, blue hour becomes the visual anchor. Dusk, blue hour, and curfew are three Cycles-baked people-free asset sets from one immutable v5 geometry/camera registration. Blue hour becomes authoritative at 20:00 and curfew at 22:00; each target prefetches at 19:50 and 21:50 and replaces the prior complete set through a 750 ms crossfade only after every target texture is ready. A generation token rejects stale jumps, restarts, and hydration completions. Failed transitions discard the partial target, retain the current complete state, diagnose observably, and retry at the next synchronization or reload; initial-state failure is visibly fatal.
...
1. Commit and read back the documentation-only entry package and this mission-legibility amendment with separate requester authorization. Record the v5 recipe/runtime identities, rejected-greybox recovery boundary, preserved-topology schema, invalidated artifacts, authored population/security boundary, OPEN items, and proof gates before production changes.
2. Add red-first contracts for the new recipe/runtime identity and the preserved-topology dataset. Version the two roads, three alleys, crossing, safehouse threshold/court, traversal loops, anchor clearance discs, and the recovered requester 24-point collision fixture with original stable-v4 provenance. The route validator must fail on footprint/route or footprint/disc intersection, any changed Class A verdict, or any Class B point lacking a recorded v5 greybox result; it must not enforce the obsolete Class B verdicts.
3. Preserve the rejected v5 coverage recipe as recoverable revision evidence, then build the corrected v5 mission-staged greybox from named Neo Tokyo 2 masses. Split each of the two hero-dominant compounds into three or four independently source-fingerprinted parcels with real street seams; pull street walls toward protected route edges without intersecting them. Place the tallest Hidzu destination at `entrance.logistics.public`; stage neutral checkpoint, transit/bench/timetable, café, terminal, hide-recess, Needle, and exact figure placeholders from their anchors. Add an unreachable perimeter backdrop. Render the fixed `1440×900` raw hero, overview, route/anchor overlay, building mask, per-identity segmentation, and mission-legibility key. Require route/probe green, density ratio `0.95–1.05`, at least ten identities at `≥0.005` each, no identity `>0.15`, and a requester-pointable answer to all five questions; stop for massing approval.
4. After massing approval, author facade identities and lighting: detailed warm/cold windows, readable labels, two to four controlled cool-white/cyan fixtures, sparse red warnings, one dominant large Hidzu Corporation landmark, visual restricted-area treatment, readable cool shadow fill, and continuous wet-ground response under every emitter. Preserve named source provenance and avoid generic neon or random rooftop filler.
5. Render one Cycles Metal blue-hour hero at `1440×900`, public-crossing centre `{29,22}`, zoom `2.9`, 45° azimuth, and 30° elevation. Present `reference | candidate | delta notes` plus a verified 200% road/reflection crop and amended two-sided visual metrics. Stop for requester hero approval.
6. After hero approval, render aligned `6400×3600` dusk, blue-hour, and curfew people-free masters at render zoom `2.0`, target `{29,22}`. Rebuild all desktop/mobile backgrounds and 12–16 per-identity foreground slices, each with its own depth anchor. Rebaseline collision, placement metadata, state hashes, registration tests, visual masks, and regression frames.
...
Produce the v5 recipe and runtime identities; recoverable rejected-greybox revision; preserved-topology dataset; exact 24-point route fixture; route/anchor/probe report; versioned reference building mask and density report; occlusion-aware per-identity masks/report; raw and annotated mission-legibility greybox evidence; source/component provenance for every 12–16 building identity; independent cutout/depth metadata; perimeter-backdrop declaration; checkpoint, transit, café, terminal, hide, and Needle placeholder/slot manifests; facade/window/sign/light inventory; one large Hidzu Corporation destination; restricted-area visual set; palette/value and wet-material guides; component-removal fingerprints; three aligned people-free environment exports; schema-v2 state/profile manifest; measured transfer/decode/residency report; and fixed live evidence. Existing non-emissive route signage remains GET-214-owned. T5 adds no cyan wayfinding or synthetic facade/building replacement.

### World/UI/audio/George feedback

World feedback prioritizes current objective/action, actor, observation/threat, traversal/entrance, building identity, then ambience. Cold shadow fill preserves facade and road detail. Each practical/emissive source has a credible local ground response. Camera coverage uses truthful restrained geometry and never paints buildings broadly cyan; its LED, IR glint, pavement response, and Observation geometry agree. Needle is neutral amber/warm-white while patrolling and crimson only while verifying or pursuing. The restricted-area treatment communicates control through barrier, warning, sign, guard, and drone placement but makes no mechanical trespass claim. T9 owns HUD implementation; GET-208 owns security/civilian behavior. George's canonical cyan-core orb remains a separate private live layer and is never baked into the city.

### Failure and recovery

The massing gate fails if a footprint or mission placeholder intersects a protected route/clearance, any Class A verdict changes, any Class B point lacks a fresh observation, a traversal loop breaks, building fraction misses `0.95–1.05`, fewer than ten identities contribute `0.5%` each, one identity exceeds `15%`, the destination/checkpoint/blend/hack/sneak read is not pointable, cutout ownership collapses into merged walls, or the requester rejects the scene. Old Class B results remain provenance and may not preserve deleted random-NPC blockers or consumable interstitial space by accident. The hero/live gate fails on sparse isolated blocks, dark void roads, unreadable windows/facades, weak identity, random NPC placement, absent or stacked mission groups, false blend eligibility, warm beige/ochre grade, dry or patchy roads, missing emitter reflections, generic neon, broad glow, impossible lights, synthetic architecture, crushed curfew blacks, or background mass that intrudes into playable space. Missing initial art fails visibly. A failed transition keeps the current complete state; a failed render or publisher leaves the public asset root intact. Restore the last accepted recipe/artifact and correct the responsible stage instead of weakening route, metric, hash, or placement tests.
...
- No facade/material/light work before massing approval; no state/cutout/runtime publication before hero approval.
...
Depends on the accepted GET-204 source, the committed density-rebuild entry gate, and a separately committed mission-legibility amendment. The requester-authored 24-point table is recovered with exact stable-v4 provenance. `OPEN-LAYOUT-006` blocks massing acceptance until all 14 Class A outcomes remain green and all 10 Class B points have fresh corrected-v5 results frozen with requester massing approval. `OPEN-ART-006` owns the provisional `0.95–1.05` building-mask/fraction tolerance; `GDR-ART-018` fixes the identity-contribution and five-question gates. `OPEN-PERF-001` owns the new measured publication budget. `OPEN-CIV-001` retains delivery/broader behavior tuning without reopening the T5 minimum; `OPEN-SEC-001` remains GET-208 tuning. `OPEN-ART-005` remains actor-light work. T5 blocks downstream final visual integration until its live acceptance. Raw Neo Tokyo 2 source remains outside Git.

### Canonical decisions/spec sections

Implements `GDR-SET-003`, `GDR-SET-005`, `GDR-SOC-001`, `GDR-SOC-003`, `GDR-SUR-008`, `GDR-SUR-011`, `GDR-SUR-012`, `GDR-SUR-013`, `GDR-CIV-002`, `GDR-CIV-003`, `GDR-ART-001`, `GDR-ART-002`, `GDR-ART-003`, `GDR-ART-012`, `GDR-ART-013`, `GDR-ART-014`, `GDR-ART-015`, `GDR-ART-016`, `GDR-ART-017`, `GDR-ART-018`, `GDR-REM-011`, `GDR-SUP-004`, and the outdoor boundary in `GDR-PROD-004`. Canonical detail is in [[20 Setting & Worldbuilding]], [[30 Art Direction (MVP)]], [[31 GET-204 Visual Rebuild Quality Contract]], [[42 Surveillance, Security & Civilian Behavior]], [[80 Day-Night Cycle]], [[35 Narrative Alignment]], [[47 Social Feed]], [[03 Lore/Plot Bible]] sections 3, 8, 11, and 12, and [[04 Engineering/Roadmap]] Gate 5.

### Human-play acceptance

The first requester gate is the mission-staged greybox at the fixed `1440×900` hero camera: locked blend, raw candidate, annotated legibility key, versioned building mask/delta, per-identity segmentation/report, candidate/reference building fraction in `0.95–1.05`, at least ten identities at at least `0.5%` each, no identity above `15%`, green route/anchor report, unchanged 14 Class A outcomes, and fresh 10 Class B observations. The requester must point to destination, obstacle, blend, hack, and sneak in the raw frame; the accepted greybox freezes Class B expectations. Only then may facade/lighting work begin. The second gate is the blue-hour hero/reference/delta comparison and 200% road crop. The final gate covers live clean-world/current-HUD frames, aligned three-state strip, overview, both profiles, three occlusion sites, actor-foot stability, and the `0.60→3.25` zoom sweep at `1280×720`, `1440×900`, `1920×1080`, and automatic mobile `390×844`. It must prove all invariant routes/anchors, 12–16 independent depth slices, the Hidzu destination/checkpoint, exact nonblocking transit/café groups, truthful blend availability, one public guard plus Needle, readable cold/warm windows, sparse red, continuous wet reflection, initial state, both prefetches/boundaries, direct jumps, rewinds, stale completion, failure retention, 750 ms crossfade, texture disposal, and measured one-/two-set residency. The requester must agree that the live scene materially matches the locked mission-density and wet-noir references; validators do not substitute for that judgment.

### Documentation and validation obligations

Atomically update the Decision Register, Review Queue, Art Direction, this complete Linear source, GET-204 historical boundary, Surveillance/Civilian specification, Content Matrix, Day-Night, Roadmap, MVP Readiness, Architecture status boundary, implementation plan, progress note, and live Linear description before corrected production work. Add red-first tests for recipe/revision recovery, preserved topology, route/anchor and placeholder intersection, 14 fixed Class A probes, fresh/frozen Class B observations, occlusion-aware identity contributions, mission-placeholder/slot ownership, 12–16 identity slices, authored population/security cuts, manifest/registration regeneration, and staging rollback. Extend the fixed-frame delta validator with the versioned building mask, provisional `0.95–1.05` density band, at least-ten/`0.5%` identity floor, and `15%` identity cap; retain the shadow, road, amber, cyan, luminance-spread, and `p05` gates for later hero/states. Rebaseline every invalidated state, cutout, collision, placement, payload, and regression artifact deliberately. After visual approval run relevant asset validators, lint, build, Jest, coverage above 80%, and the guided Level 0 agent playtest. The amendment commit and accepted implementation commit require separate explicit authorization; GET-205 remains non-terminal until requester verification of the committed build.
