...
- Current layout dimensions, loop geometry, street widths, anchors, safehouse seed, and camera composition constraints come from [[32 GET-205 Reference-Native Layout Contract]] under `OPEN-LAYOUT-007`; actor movement and follow tuning remain separately open. T3's existing geometry is replaceable implementation, not authority.
...
Depends on T2. Blocks T3A, T4, T7, T8, T9, and T10. Current critical acceptance blockers are `OPEN-NAR-007`, `OPEN-TIME-001`, `OPEN-MOV-001`, `OPEN-CIV-001`, `OPEN-SEC-001`, `OPEN-LAYOUT-003`, `OPEN-LAYOUT-007`, and `OPEN-SAFE-001`. High blockers are `OPEN-NAR-013`, `OPEN-MOV-002`, and `OPEN-MOV-003`. These items do not block ticket start after T2; a recorded recommendation encoded before approval remains a reversible provisional trial with seam, live proof, and rollback.
...
- [[32 GET-205 Reference-Native Layout Contract]] sections 6.1–6.9 supply the mandatory exact reversible seed: `44×38` bounds; life, approach, public/private HIDZU, safehouse, service, and sneak polygons; dynamic gate; parcel footprints/heights; authored-presence slots; relocated anchors; camera boxes; and 22 static plus four dynamic probes. `OPEN-LAYOUT-007` keeps only this seed provisional until requester plan/greybox approval.
...
- The documentation entry package must be committed and read back separately before production art or runtime changes. No documentation or implementation commit is implied by plan approval.
...
The T5 lifecycle is `COMMITTED_TECHNICAL_BASELINE → REFERENCE_NATIVE_DOCUMENTATION_COMMIT → EXACT_PLAN_MANIFEST → DIMENSIONED_PLAN_AND_RAW_GREYBOX_GATE → SOURCE_FACADE_HERO_LOCK → THREE_STATE_EXPORT → CUTOUT_PROFILE_MANIFEST_REGEN → LIVE_SIDE_BY_SIDE_ACCEPTANCE → ACCEPTANCE_COMMIT → COMMITTED_BUILD_VERIFICATION`.

One plan manifest drives bounds, routes, parcels, building footprints, gate collision, anchors, proof starts, camera constraints, and the replacement probe fixture. The validator rejects any footprint/fixture that intersects a route or required `0.32` clearance, any static verdict that differs from the 22 specified outcomes, failure of the four dynamic gate/bypass/reset probes, a disconnected required anchor, or drift from the exact seed without a canonical amendment. Occlusion-aware masks at `720×450` require at least ten hero identities contributing at least `0.005` of the full frame each and none above `0.15`; declared counts and token slivers are invalid. The raw `1440×900` greybox must answer destination, obstacle, blend, hack, and sneak without labels and satisfy the plan's normalized camera boxes. No facade, material, lighting, state bake, cutout, or runtime publication starts until requester plan/greybox approval closes `OPEN-LAYOUT-007`.

After plan approval, blue hour becomes the visual anchor. Dusk, blue hour, and curfew are three Cycles-baked people-free asset sets from one immutable v6 geometry/camera registration. Blue hour becomes authoritative at 20:00 and curfew at 22:00; each target prefetches at 19:50 and 21:50 and replaces the prior complete set through a 750 ms crossfade only after every target texture is ready. A generation token rejects stale jumps, restarts, and hydration completions. Failed transitions discard the partial target, retain the current complete state, diagnose observably, and retry at the next synchronization or reload; initial-state failure is visibly fatal.
...
1. Commit and read back the documentation-only reference-native authority package with separate requester authorization. Record `GDR-ART-019`, v6 identities, exact-seed ownership, superseded v4/v5 constraints, invalidated artifacts, authored population/security boundary, `OPEN-LAYOUT-007`, and proof gates before production changes.
...
3. Preserve v4/v5 as recoverable evidence, then render the dimensioned top-down plan, parcel/anchor overlay, and exact material-free raw greybox with simple mission placeholders. Produce reference/candidate, route/anchor overlay, per-identity masks, source/proportion feasibility, and a hidden answer key. Require all static/dynamic checks green, at least ten identities at `≥0.005`, none `>0.15`, all normalized camera boxes satisfied, and a requester-pointable answer to all five questions; stop for plan approval.
4. After plan approval, build the source-derived v6 scene and author facade identities/lighting: detailed warm/cold windows, readable labels, controlled cool-white/cyan fixtures, sparse red warnings, one dominant HIDZU tower, real restricted-area treatment, readable cool shadow fill, and continuous wet-ground response under every emitter. Preserve named source provenance and avoid generic neon or random rooftop filler.
5. Render one Cycles Metal blue-hour hero at `1440×900`, 45° azimuth and 30° elevation using the exact camera constants frozen from the approved v6 plan boxes. Present `reference | candidate | delta notes` plus a verified 200% road/reflection crop and two-sided visual metrics. Stop for requester hero approval.
...
Produce the v6 recipe/runtime identities; exact plan manifest; dimensioned plan; parcel/anchor overlay; route/anchor/dynamic-gate report; 22-static/four-dynamic probe report; camera-box report; source/proportion feasibility; occlusion-aware per-identity masks/report; raw and annotated mission-legibility greybox evidence; source/component provenance for every building identity; independent cutout/depth metadata; perimeter-backdrop declaration; checkpoint, transit, café, terminal, hide, and Needle slot manifests; facade/window/sign/light inventory; one dominant HIDZU destination; restricted-area visual set; palette/value and wet-material guides; component-removal fingerprints; three aligned people-free environment exports; schema-v2 state/profile manifest; measured transfer/decode/residency report; and fixed live evidence. Existing non-emissive route signage remains GET-214-owned. T5 adds no cyan wayfinding or synthetic facade/building replacement.
...
The plan gate fails if implementation drifts from the exact seed without a canonical reason, a footprint/fixture intersects a route or anchor clearance, any replacement probe fails, the barrier does not block/open/reset correctly, the bypass is disconnected, any required anchor is unreachable, a camera box misses, fewer than ten identities contribute `0.5%` each, one identity exceeds `15%`, destination/obstacle/blend/hack/sneak is not pointable, source/proportion feasibility fails, or the requester rejects the plan. The hero/live gate fails on reference-adjacent rather than reference-native composition, decorative-only gate behavior, dark void roads, unreadable windows/facades, weak identity, random NPC placement, false blend eligibility, warm beige/ochre grade, dry/patchy roads, missing reflections, generic neon, synthetic architecture, crushed curfew blacks, or background mass intruding into playable space. Missing initial art fails visibly. A failed transition keeps the current complete state; a failed render/publisher leaves the public root intact. Correct the responsible stage instead of weakening plan, route, probe, hash, or placement tests.
...
- No facade/material/light work before plan/greybox approval; no state/cutout/runtime publication before hero approval.
...
Depends on the recoverable GET-204 named-source baseline and the separately committed reference-native authority package. `OPEN-LAYOUT-007` blocks facade work until the exact v6 seed passes dimensioned-plan/raw-greybox requester approval; it does not permit old-coordinate fallback. `OPEN-PERF-001` owns the new measured publication budget. `OPEN-CIV-001` retains delivery/broader behavior tuning without reopening the T5 minimum; `OPEN-SEC-001` remains GET-208 tuning; the T5 gate collision is real while detection/trespass behavior stays outside this ticket. `OPEN-ART-005` remains actor-light work. T5 blocks downstream final visual integration until live acceptance. Raw Neo Tokyo 2 source remains outside Git.
...
Depends on T9A and existing GET-210 city/content prerequisites; blocks T10B and final GET-139 acceptance. Relevant provisional blockers include `OPEN-TIME-001`, `OPEN-CIV-001`, `OPEN-LAYOUT-003`, `OPEN-LAYOUT-007`, `OPEN-NAR-014`, `OPEN-AUD-001`, `OPEN-LOC-001`, `OPEN-ACC-001`, and `OPEN-PERF-001`. Approved exact boundary times, route names, grounding values, difficult relief, and threshold counts cannot be changed by tuning.
