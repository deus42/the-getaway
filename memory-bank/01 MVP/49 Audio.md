...
Audio makes the watched city and every important state transition legible without asking the player to stare at the HUD. It grounds movement and place, distinguishes network concern from pursuit and drone verification, and gives interaction, consequence, safehouse, failure, and completion their own authored weight. Level 0 audio is required feedback, not optional polish. This implements `GDR-AUD-001`.

## 2. Player-visible verbs

- Hear the outdoor city and safehouse contexts.
- Hear protagonist movement and explicit interaction responses.
- Hear distinct camera/network, verifier-drone, curfew, consequence, objective, UI, failure, and completion cues.
...
1. The safehouse opening establishes the grounded city/safehouse sound context without masking George, UI, or critical operation cues.
...
4. Cameras and public/security behavior remain readable in the world; `Suspicious`, `Pursuit`, drone approach, and verification each provide clearly differentiated authored warnings.
5. Curfew transition, connected-terminal use, cache release, medkit recovery, consequences, safehouse return, Rest, and outbound validation each trigger only their declared cue family.
...
- `Clear`, `Suspicious`, `Pursuit`, drone approach, drone verification, and recovery each remain distinct event states; audio cannot collapse or reorder the network state machine.
...
- Retry restores the operation-departure semantic state and clears post-departure audio ownership so no stale alarm, drone, interaction, or completion cue persists.
...
- Drone, network transitions, objectives, and dialogue UI are separate required cue contexts; their provisional differentiation, priority, and mix follow `OPEN-AUD-001` until human acceptance resolves it.
...
- [[43 Health, Failure & Recovery]] supplies physical consequence, Paranoia threshold, failure, recovery, and Retry events.
- [[44 Safehouse, Save & Retry]] supplies waiting, Rest, save, departure, terminal validation, and restoration events.
...
- Distinct warnings improve the player's ability to identify source, urgency, current network state, drone verification, curfew, objective transition, and exact consequence.
- UI and reading-surface audio confirms focus/selection without issuing movement or interaction beneath an overlay.
- Retry and New Game clear stale audio presentation so restored state is not contradicted by a previous attempt's alarm, drone, deadline, failure, or completion sound.
...
- Camera concern, `Suspicious`, `Pursuit`, recovery, drone approach, and drone verification use distinct authored feedback paired with truthful world/HUD state.
...
- Missing audio cannot hide or alter Retry/New Game actions; visual/text feedback remains complete.
- Retry stops or clears post-departure presentation and restarts only the ambience/cues appropriate to the restored departure state.
...
- Map every cue to a stable semantic event, required world/HUD/text companion, pause behavior, Retry/New Game cleanup, localization/subtitle need, and the source/licensing evidence required by the approved or explicitly provisional `OPEN-AUD-001` baseline. Allowed overlap and priority remain provisional until that decision is accepted; provisional cues cannot close production-audio acceptance.
- Provide distinct authored cues for camera concern/network transitions, verifier-drone approach/verification, objectives, dialogue UI, terminals, medkit recovery, Health/Paranoia consequence, Rest, failure causes, transit validation, debrief/progression, and demo closure.
...
- No stale alarm, drone, curfew, movement, dialogue, failure, or completion loop after pause, Retry, New Game, or state restoration.
...
- Future campaign audio may extend the same semantic-event, provenance, licensing, priority, accessibility, and Retry cleanup contract only after a new recorded decision.
...
- Operate all three terminals, recover medkits, take one authored consequence, Rest, validate transit, fail an attempt, Retry, and complete the demo; each required event has distinct synchronized feedback and no stale cue.
...
`T10` (`GET-210`) owns authored Level 0 audio, source/licensing, integration, localization/subtitle completion, and end-to-end acceptance. `T3` (`GET-203`), `T7` (`GET-207`), `T8` (`GET-208`), and `T9` (`GET-209`) supply authoritative runtime events; `T5` (`GET-205`) and `T6` (`GET-206`) supply world/actor presentation that audio must reinforce.
