...
- inspect the exact deterministic requirement and complete component math before choosing;
...
5. During or after infiltration, authored interactions mount the shared `Level0CheckBreakdown` in `preview` mode before every deterministic choice and reuse it in `result` mode after commitment.
6. On return, Lira reads the actual outcome ledger: contacts consulted, timing, camera handling, pursuit, Health loss, Paranoia, medkits, and optional evidence.
...
2. compute and present the deterministic check breakdown if present;
...
- Every nonterminal failed check changes the situation through its declared time, Paranoia, Health, access, position, objective-precision, or surveillance effect and communicates that worse-but-real path. Only the final failed capture-escape choice may end the attempt.
...
- change Health or Paranoia through a clearly communicated event, except that dialogue is never a Paranoia-recovery source;
...
- Requirement explanations use the mounted `Level0CheckBreakdown` with `preview | result` presentation and show the relevant attribute, skill, fact, modifier, Paranoia penalty, requirement, total, and outcome.
...
- Author dialogue variants for every `ColdIronEvidenceState`, pursuit, Health loss, Paranoia, camera history, contacts consulted, and deadline pressure. Recognition and explicit five-minute copying remain separate actions.
- Validate that every nonfatal check node has a reachable, concrete fail-forward effect and that preview/result math is identical.
...
6. Preview and result show identical exact math for every check; only the final failed capture-escape option can end the attempt.
7. Lira’s debrief accurately distinguishes contacts skipped, route, camera history, pursuit, Health loss, Paranoia peak, and Cold Iron evidence state.
8. English and Ukrainian runs commit identical state changes for the same selections.

## 16. Owning Linear ticket

- System infrastructure: `T9` (`GET-209`) plus `T9A` (`GET-213`) — mounted check previews/results, fail-forward validation, Cold Iron evidence, George explanations, departure readback, and failure legibility.
