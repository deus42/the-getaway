...
| Paranoia tiers and ability locks | GET-216 tier/lock foundation implemented; surveillance integration pending | Internal 0–100 derives Calm/Uneasy/Shaken/Breaking and terminal Breakdown; the number is absent from normal UI. One George entry is recorded per newly crossed tier, fragile abilities lock at declared tiers, hardened abilities stay lit, and 100 stages surrender as `failure.breakdown`. `OPEN-PAR-001` and T8A own observation rate/overlap/sampling/caps. |
...
| Autosave | Version-3 cover/ability payload implemented; final checkpoint cadence deferred | New Game and Continue round-trip exact cover, ability, research, Paranoia, fact, clock, and runtime state through the independent validated key. Transient UI pause owners are stripped; retired numeric/package/combat/inventory ownership is excluded. |
