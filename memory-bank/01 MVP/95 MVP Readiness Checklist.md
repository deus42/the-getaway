...
| Camera/zoom contract | V6 hero camera frozen; runtime follow tuning pending | The requester-approved hero baseline is target `{21.177121,17.126983,1.621967}`, orthographic scale `25.687085`, 45° azimuth, and 30° elevation. The old `2.00`/`3.25` trial and `{29,22}` framing do not own v6. `OPEN-MOV-003` retains only player-follow and runtime zoom-range tuning around the fixed composition. |
...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Shared outdoor layout contract | V6 plan, camera, and greybox approved/frozen | `GDR-ART-019` retires the old grid/coordinates. After rejecting the first roof-dominated raw frame, the requester approved the bounded amendment and amended raw greybox on 2026-08-13. `OPEN-LAYOUT-007` is resolved; the exact seed, camera, raw baseline, and 22-static/four-dynamic probe fixture in [[32 GET-205 Reference-Native Layout Contract]] are frozen. |
...
| Hidzu Corporation identity pass | Real-source hero correction awaiting authority commit | Wet-blue-black v4, rejected v5, and the envelope-led v6 dressed rounds remain technical/failure evidence. The approved v6 plan structurally stages the life street, HIDZU street, real checkpoint, tower, transit/café presence, terminals, and sneak/service seam. Read-only scene diagnosis proved that camera-visible procedural parcel envelopes obscure already-instanced Neo Tokyo 2 meshes; a fresh dependency audit also finds `125` missing images in the immutable GET-204 donor master and `13` in the rejected saved v6 hero, so session-only remapping is not durable. After the amended layout-closure/source-hero authority commit, the next candidate derives a new scene without mutating either input, reopens with zero missing images, replaces the visible shells with 27 manifest-backed real source assemblies, builds the HIDZU tower/gate signs as readable dimensional geometry, adds bounded life-street/junction clutter, and applies the existing disciplined wet-blue-black photographic finish without moving the frozen layout/camera. |
...
| Graphic surveillance noir | Look family established; v6 hero pending | The wet-blue-black family, two-sided luminance/cyan gates, continuous wetness contract, and semantic amber/cyan/red boundaries remain authoritative. V4 is rejected for crushed darkness/sparse identity and v5 for reference-incompatible geography. V6 has passed its exact plan/greybox gate; a brighter detailed source-derived blue-hour hero, then aligned three-state live proof, remain pending. |
...
Historical GET-205 technical evidence: the 2026-08-07 publication and 2026-08-10 v4 three-state work prove recoverable publication, profile, fixed-frame, and generation-safe transition seams only. On 2026-08-11 the live v4 candidate measured `52.05%` near-black pixels and `2.68%` detail-edge coverage versus the locked blend's `39.93%` and `4.29%`; the requester rejected its massing, light distribution, facade/window identity, and NPC presentation. It is not v6 acceptance evidence. The requester approved the reference-native v6 plan/greybox on 2026-08-13; its raw baseline SHA-256 is `ddf5a224da0eec2fe8af5b1c0bf2f7ca8f5e618080ef1d63b4a18c4c72aa7369`. The later v6 dressed rounds preserved the plan and lighting gates but are rejected because camera-visible procedural envelopes reduced the real kit buildings to flat painted shells. The consolidated mockup bundle proves only the assembly-method/detail contrast; its altered coordinates, wide donor slots, plane/decal signage, and prototype clearing are not production authority. The real-source blue-hour hero with immutable-donor/zero-missing-image proof, all 27 parcel provenance records, dimensional readable HIDZU sign geometry, zero visible envelopes, source-only silhouette IoU `>=0.85`, facade/road crops, bounded clutter, three-state live evidence, closeout checks, and final requester acceptance remain pending.
