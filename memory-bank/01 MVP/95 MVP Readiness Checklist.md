...
| Hidzu identity pass | Locally validated; commit, requester acceptance, and runtime promotion pending | GET-205 hash-locks the GET-204 master and aligned art manifest, keeps the stable recipe/layer contract, and applies a provisional local Hidzu treatment with nine measured material overrides, 24 measured gameplay/civic additions, six motivated practical-light sites, three aligned schedule states, spatially and byte-identical T4 semantic masks, exact 27-anchor validation, and 17 fixed captures. The current Blender 5.0.1 `all` run contains 154 generated objects, 87 registered outputs, and 88 files total; staging, the immutable published run, post-swap readback, and the atomic `current` pointer all passed export/capture validation. Validation explicitly requires and physically checks every registered output including the authoring `.blend`, manifest-derived surface transforms, 24 exact grammar bindings, seven visible public messages against their actual rendered font bodies, three color-independent state-cue groups, independently recomputed palette coverage, and all 27 anchors. Fifteen destructive mutations plus four publication/rollback cases were rejected. The ignored reference/output remain `local-evidence`, `runtimeReady: false`, with `level0-greybox` fallback because acquisition-specific entitlement is unavailable. Full repository closeout, commit, and requester judgment remain required. |
...
| Graphic surveillance noir | Partially evidenced | Provisional GET-205 frames add sodium practicals, institutional wayfinding, public screens, cameras, connected terminals, identity-control shapes, scarce cyan, and a constrained crimson threat hook without changing city geometry or reproducing zoom corruption. Correct scene-linear conversion now preserves charcoal/umber masses and readable schedule separation; Clear, Suspicious, and Pursuit also differ by silhouette rather than color alone. The inherited district still reads sparse in places, the actors and live surveillance behavior belong to later tickets, and the generated art remains local proof rather than runtime presentation. Requester visual acceptance at the fixed matrix remains the gate. |
