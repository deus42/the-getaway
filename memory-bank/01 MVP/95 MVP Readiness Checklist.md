...
| Decision Register | Ready for requester review | 111 unique current/removed/postponed/superseded decisions map to canonical documents and GET-201–GET-210, including the Approved in-game Bible rule; unresolved choices retain explicit acceptance gates and never render in the player reference. |
...
| Content/state matrices | Ready for requester review | 13 mission states, 9 objectives, 16 facts, 11 checks, 3 terminals, 5 failure IDs, 19 acceptance cases, the end-to-end transition ledger, and 48 explicit `OPEN-*` blockers validate without unknown references. |
...
| Linear program alignment | Ready for requester review | GET-201–GET-210 were fully rewritten from the corrected canonical program on 2026-08-06. All ten passed semantic readback after only Linear's automatic issue-link, bullet, and ordered-list normalization; status, label, title, priority, project, parent, related, blocker, and dependency metadata had zero changes. |
...
| Documentation commit | Blocked on requester authorization | The original GET-201 package is committed separately at `b50a4cd5290490cc8ab8c3521a2c22acaa1afdce`. The audit-backed authoring expansion and corrected in-game Bible specification are intentionally uncommitted. The player-facing runtime cannot begin until this corrected package passes review and receives a separate explicitly authorized documentation commit. |

The program entry gate opened with the original reviewed documentation commit. This audit-backed expansion keeps GET-201 nonterminal but does not invalidate already delivered implementation gates. Open decisions follow [[14 Specification Review Queue]]: they block final acceptance of affected surfaces, while documented reversible provisional trials may be built for live evaluation.
...
| Paranoia | Implemented foundation; exposure integration/tuning blocked | 0–100, exact all-check thresholds, honest threshold re-arming after recovery, exact localized source/amount feedback, departure-derived Retry treatment, fatal 100, no passive curfew mutation, Rest −40, and provisional −10/−5 recovery presets are tested. The approved source shape is sustained valid observed/Pursuit exposure; T8 still owns its deterministic accumulator and `OPEN-PAR-001` rate/overlap/sampling/caps. `OPEN-PAR-002` and T8/T10 integration also block acceptance. |
