...
| Unchanged-kit Blender city gate | Locally implemented; requester acceptance and runtime promotion blocked | Blender 5.0.1 generation against committed `level0-tokyo-greybox-v3` composes nine native-scale buildings and 15 gameplay props in one scene, with six calibrated default/0.60 captures plus overview. The ignored local export contains 11 aligned layers, 66 tiles, five semantic masks, 27 contract-checked anchors, greybox fallbacks, 23,643,936 measured bytes, a maximum projection error of 0.000378 px, and verified renderable bounds inside the canvas. Independent visual review rated the composition 9/10, but requester acceptance remains required. Acquisition-specific entitlement is unavailable, derivatives remain ignored `local-evidence`, the runtime still uses the greybox, and Hidzu treatment has not started. |
