...
| Street clock and audio | Partially evidenced; provisional acceptance blocked | The runtime persists exactly `clock.2100`, `clock.2130`, `clock.2200`, and `clock.2330`; ordered presentation survives large jumps and announces again after Restart Attempt while hydration does not replay history. Human-control desktop proof covers all four crossings, curfew treatment, bilingual PA copy, Transit Road/Market Ring/Outer Space signage, ambience captions, civilian thinning, and a clean reload/console. Three synthesized spatial threshold sources and their stage falloff are automated and live-connected; final source/mix/priority, caption, civilian, layout, and lighting values remain provisional under the named `OPEN-*` decisions, and all three sources still need one accepted human route. |
