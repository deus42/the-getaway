...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Shared outdoor layout contract | Corrected provisionally; approval blocked | T4's first deterministic overview exposed the v1 sparse-board topology and reopened T3. Typed `level0-tokyo-greybox-v2` keeps the 84×60 bounds, three loop paths, 20 semantic surfaces, 27 stable anchor IDs, masks, and art layers while narrowing road bands to four units, capping authored plazas at 120 square layout units, compacting the logistics interaction space, and raising authoritative building-footprint coverage from 16.7% to 30.3% across eight primary and four attached gameplay masses. Footprints are overlap/bounds validated; loops and required anchors are validated with the runtime's 0.32 collision clearance. Exact values remain `OPEN-*`; requester/live route acceptance is still required. |
...
| Unchanged-kit Blender city gate | In progress; requester acceptance blocked | Blender 5.0.1 local generation now proves one camera/scene/scale system against the v2 contract and records exact source/archive hashes, transforms, and an external-only raw-asset boundary. The first overview correctly failed the sparse-board gate; the v2 rerender is materially denser and removed raised collision-footprint plates, but fixed captures, final unchanged-kit composition review, and acquisition-specific entitlement evidence for any committed derivative remain outstanding. Direction B is still a T5 Hidzu-treatment blocker, not a T4 unchanged-kit blocker. |
