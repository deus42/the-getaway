...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Shared outdoor layout contract | GET-204 baseline recoverable; first GET-205 v5 massing rejected | Stable route purposes, required locations, semantic IDs, direct movement, and projection helpers remain useful. The accepted GET-204 recipe and its sixteen source-measured blockers remain immutable/recoverable. `GDR-ART-017` authorizes v5 to consume dead interstitial block gaps while preserving the executable route/anchor dataset. The original 24 probes are recovered: 14 Class A outcomes stay invariant and 10 Class B points are freshly observed. The first v5 greybox passed those checks and the `0.9076` density floor but was rejected because only two identities materially occupied the hero and the mission anchors were not staged. `GDR-ART-018` now requires the five-question legibility read, at least ten meaningful hero identities, and a 15% per-identity cap. |
...
| Hidzu Corporation identity pass | First v5 greybox rejected; mission-legibility amendment pending commit | GET-205 commit `c1f7cda`, the v3 source, wet-blue-black v4 pipeline, and rejected first v5 recipe/render remain recoverable technical evidence. The first v5 greybox corrected whole-frame coverage but rendered as anonymous mass: `v5.public.market` contributed `37.69%` and `v5.service.workshop` `23.20%`, while fourteen declared identities were effectively hidden. The corrected gate stages the actual destination, checkpoint, blend crowd, terminals, hiding route, café, and Needle; requires ≥10 identities at ≥0.5% of the hero each and ≤15% for any identity; and targets the `0.95–1.05` density mid-band. Facade work remains blocked. |
...
| Street clock and audio | Environment transition salvage evidenced; v5 revalidation pending | The current runtime proves the `19:50`/`21:50` prefetch, `20:00`/`22:00` selection, 750 ms generation-safe transition, and retained-current-state failure seam. GET-205 v5 must revalidate that behavior against regenerated assets. T5 replaces random ambient allocation with the exact three-person transit group and two-person café group; `blend.public_queue` follows visible occupancy/schedule while `blend.delivery_activity` remains unavailable. GET-208 owns delivery population and broader behavior acceptance. Final source/mix/priority, captions, and remaining tuning stay provisional under their named open decisions. |
| Graphic surveillance noir | Look family established; corrected v5 mission-massing gate pending | The wet-blue-black family, two-sided luminance/cyan gates, continuous wetness contract, and semantic amber/cyan/red boundaries remain authoritative. The live v4 execution is rejected for crushed darkness, sparse massing, weak facade/window detail, and concentrated rather than distributed light identity. The first v5 greybox is also rejected: numeric coverage without destination/obstacle/blend/hack/sneak legibility is not acceptance. v5 must pass the corrected route-safe mission-staged gate, then a brighter detailed blue-hour hero, then aligned three-state live proof. Broader surveillance-device behavior remains GET-208-owned. |
