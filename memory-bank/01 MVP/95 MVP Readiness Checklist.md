...
|---|---|---|
| Cover-select in under one minute | Superseded scope; pivot re-implementation pending under T7A/GET-216 | New Game becomes the four-cover select (one playable social-forward, three visibly disabled, zero numbers) per `GDR-PC-006`; the delivered numeric creation remains historical evidence. Original evidence: callsign, one of four appearances, exact +4 attribute/+6 skill budgets, caps, practical descriptions, review, and confirmation before creating the run at `L0_SAFEHOUSE_INTRO`. Component proof covers invalid/cancel/confirm paths and exact save state; fixed-viewport human timing/visual review is still unavailable and cannot be inferred from tests. |
...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Four covers (historic: appearances and callsign) | Superseded scope; cover presentation pending under T7A | Historic evidence: normal creation selects and persists one grounded appearance plus a normalized player callsign; there is no production sample default. The safehouse actor, HUD lane, Character panel, autosave, and Restart Attempt payload use the same identity. Final dialogue/debrief identity belongs to T9/T10; `OPEN-RPG-005` and visual requester review still block acceptance. |
| Binary abilities (historic: four attributes and eight skills) | Superseded scope; ability catalog pending under T7A and `OPEN-ABL-001` | Historic evidence: exact budgets, creation caps, long-term caps, localized practical copy, two distinct normal-created builds, and strict hydration validation are evidenced without background/package residue. Player-facing mission consequences remain T8–T10 integration. |
| Deterministic gates (historic: checks) | Superseded scope; gate resolver pending under T7A per `GDR-RPG-009` | Historic evidence: the pure catalog/resolver and reusable result breakdown are tested. Acceptance now also requires the identical exact math mounted as `preview` before every checked choice and `result` afterward, a validated real worse path for every nonterminal failure, and terminal failure only for the final failed capture escape. `OPEN-RPG-001`/`OPEN-RPG-004` still block numeric acceptance. |
| Character screen | Superseded scope; cover/ability panel pending under T7A | Historic evidence: the paused panel shows callsign/portrait, level/XP, attributes, skills, Health, Paranoia/penalty, points, known facts, and truthful long-term consequences. Raw Rest/resource logs are excluded; cap reasons are visible; modal focus enters, remains contained, and returns to the Character trigger. It hides check-catalog spoilers and dead package/perk/equipment/combat/faction UI; final T9 HUD treatment and live viewport review remain. |
| Safehouse research (historic: milestone XP) | Superseded scope; research action pending under T7A per `GDR-RPG-010` and `OPEN-ABL-002` | Historic evidence: stable one-shot IDs and an ordered ledger trial `50 + 50 = 100 XP`, producing one pending Level 2 without grinding or duplication. `OPEN-RPG-002` blocks acceptance; T10 owns medkit/transit trigger integration. |
| Safehouse/debrief level-up | Superseded scope; removed by `GDR-RPG-008` | Historic evidence: domain and Character UI enforce safehouse/debrief-only activation/allocation, two skill points per level, every-third-level attribute point, caps, localized feedback, exact persistence, and visible disabled/cap explanations. Final debrief trigger/carry proof remains T10. |
| Health removal | Superseded scope; removal executes under T7A per `GDR-HLT-004` | Historic Health evidence: authored event ledger, idempotency, stable source/feedback/attempt metadata, exact localized source/amount feedback, fatal 0, sourced bilingual failure overlay, Rest-to-100, and provisional 10/25/40 preset table are tested. Acceptance explicitly excludes injury states, limps, movement penalties, detection changes, and civilian reaction to Health. `OPEN-HLT-001` plus encounter assignment still block tuning. |
| Paranoia tiers and ability locks | Foundation implemented for the superseded penalty model; tier-lock conversion pending under T7A per `GDR-PAR-008`/`GDR-PAR-009` | Historic evidence: 0–100 and exact all-check penalties are implemented. Acceptance now requires surveillance-origin gain only from paired valid observation plus rule-break evidence; one-use Transit Road coffee and junction shrine actions at ten minutes/−10 each; one qualifying difficult-escape −5; no dialogue-based relief; and one George warning per attempt at 40/70/90. `OPEN-PAR-001` still owns unresolved rate/overlap/sampling/caps. |
...
| Hidzu Corporation identity pass | Requester accepted; acceptance commit authorized | GET-205 derives one people-free environment from the accepted Neo Tokyo 2 four-block master, keeps all sixteen GET-204 transforms/topology/anchors intact, bakes restrained facade identity plus a non-colliding atmospheric surround in Blender, and publishes hash-checked desktop/mobile WebP profiles. The normal Level 0 route selects GET-205 without a query flag; `visualTreatment=get204-1` remains the explicit diagnostic fallback. Live close, minimum overview, automatic `390×844` mobile, dusk, blue-hour, and curfew evidence is clean; the final fixed desktop close and overview were recaptured at true `1920×1080` with zero browser warnings/errors. The ticket remains nonterminal until the requester verifies the committed build. |
...
| Graphic surveillance noir | Named-source T5 production candidate live | The runtime combines the accepted Neo Tokyo 2 four-block geometry with Blender-baked facade-scale Hidzu identity, a dark atmospheric surround, sparse cyan technology, warm practical lighting, and geometry-stable dusk/blue-hour/curfew treatment. Oversized post-render banners, SVG identity cards, blue void, baked people, zoom composition swaps, and actor counter-scaling are absent from the active production path. Broader surveillance-device state semantics remain GET-208-owned. |
...
| Autosave | Historic RPG payload implemented; v3 cover/ability payload pending under T7A | Historic evidence: New Game and Continue round-trip exact player-created identity/build/RPG state through the independent validated key; safe transitions and exit protection persist it, transient UI pauses are stripped, and retired package/combat/inventory ownership remains excluded. Final authored checkpoint cadence remains later integration work. |
| OperationAttemptBaseline and Restart Attempt | Rename/schema integration not started; scope absorbed into T7A/GET-216 | `OperationAttemptBaseline` must capture the complete approved departure state; George must read actual departure time, contacts, Health, Paranoia, and restoration meaning; `restartAttempt` must restore it under `restart_attempt_confirmation`. Current snapshot-shaped foundation is superseded terminology, not acceptance. |
...
| Failure causes | Breakdown/deadline foundation pending conversion under T7A; new reports deferred | Midnight must list exact incomplete requirements without implying capture. Health 0 and Paranoia 100 remain factual. Capture requires the evidence-limited Hidzu Corporation report/map. Restart Attempt appears only with a valid `OperationAttemptBaseline`. |
...
Current GET-205 acceptance evidence (2026-08-07): the deterministic production publisher emits and validates 37 registered assets; the exact staged candidate passes `yarn lint`, `yarn build`, `146/146` Jest suites with `1041/1041` tests, and coverage at `81.36%` statements/lines. The guided 20-step Level 0 regression records zero console/page/network errors; its two findings remain the deferred objective-guidance and `focusObjective` harness gaps owned by later content/playtest work. Inspected live frames cover `1440×900`, true fixed `1920×1080` close and minimum overview, automatic `390×844` mobile, blue hour, and curfew. The inherited pre-four-block `art:level0:t5:all:validate` remains red at its stale GET-204 source-recipe prerequisite; the accepted manifest-driven production publisher and runtime tests are green, and rebasing that historical recipe is a separately recorded residual rather than hidden acceptance evidence.
