...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Shared outdoor layout contract | GET-204 baseline recoverable; GET-205 v5 massing gate pending | Stable route purposes, required locations, semantic IDs, direct movement, and projection helpers remain useful. The accepted GET-204 recipe and its sixteen source-measured blockers remain immutable/recoverable. `GDR-ART-017` authorizes a new v5 recipe to consume dead interstitial block gaps while preserving the executable two-road/three-alley/crossing/safehouse/loop and anchor-clearance dataset. The original 24 collision probes are recovered: 14 Class A geometry/bounds outcomes stay invariant, while 10 Class B actor/interstitial/guard-dependent points are re-probed and frozen from the requester-approved greybox rather than carrying obsolete v4 outcomes. |
...
| Hidzu Corporation identity pass | Density rebuild authorized; documentation entry gate open | GET-205 commit `c1f7cda`, the v3 source, and the wet-blue-black v4 pipeline remain recoverable technical baselines. On 2026-08-11 the requester rejected the live v4 result as too dark, too sparse, weak in window/facade identity, and cluttered by random NPC allocation. The approved replacement is a new `get205-dense-four-block-v5` recipe/runtime identity with sequential massing, hero, and final-live gates. All v4 state masters, cutouts, collision/position pins, budgets, and regression frames require deliberate regeneration after approval. No production change or implementation commit is authorized by this documentation pass. |
...
| Street clock and audio | Environment transition salvage evidenced; v5 revalidation pending | The current runtime proves the `19:50`/`21:50` prefetch, `20:00`/`22:00` selection, 750 ms generation-safe transition, and retained-current-state failure seam. GET-205 v5 must revalidate that behavior against regenerated assets. The T5 candidate removes all ambient civilians, so `blend.public_queue` and `blend.delivery_activity` must be explicitly unavailable; GET-208 owns later capacity-honest population and the `18:45`/post-`21:30`/curfew wind-down proof. Final source/mix/priority, captions, counts, and layout values remain provisional under their named open decisions. |
| Graphic surveillance noir | Look family established; v5 massing/hero/live gates pending | The wet-blue-black family, two-sided luminance/cyan gates, continuous wetness contract, and semantic amber/cyan/red boundaries remain authoritative. The live v4 execution is rejected for crushed darkness, sparse massing, weak facade/window detail, and concentrated rather than distributed light identity. v5 must first pass route-safe greybox density against the locked blend, then a brighter detailed blue-hour hero, then aligned three-state live proof. Broader surveillance-device behavior remains GET-208-owned. |
