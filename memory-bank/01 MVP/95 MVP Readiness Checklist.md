...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Four appearances and callsign | Appearance seam implemented; RPG identity incomplete | GET-206 provides four grounded portrait choices, persists `appearancePresetId` through the existing fresh-run/autosave seam, and renders each selected preset in the safehouse world. Callsign/build creation, Character/HUD/dialogue identity, authoritative Retry, and continuation proof remain T7/T9/T10 work and are not implied by this asset seam. |
...
| Incompatible rewrite saves | Technically evidenced; live review pending | The active menu never hydrates `the-getaway-state`, leaves it untouched until explicit New Game, explains incompatibility bilingually, and clears it only through that action. Current Level 0 records strictly accept one of the four authored appearance IDs; only the known provisional placeholder ID is narrowly migrated to preset 1 in otherwise compatible autosave/Retry records, while unknown future IDs are rejected. |
