...
| Camera/zoom contract | Greybox behavior implemented; final framing rejected | Follow/observation ownership exists, but the `0.78/0.60` board-first framing is not final. The complete GET-204 live candidate calibrates both close street-first normal play and the composed full-city manual overview. |
...
| Gate | State | Acceptance evidence required |
|---|---|---|
| Shared outdoor layout contract | Mission skeleton implemented; accepted visual candidate uses replacement collision | Stable route purposes, required locations, semantic IDs, direct movement, and projection helpers remain useful. The exact `84×60` nine-block geometry is rejected. The accepted GET-204 checkpoint replaces its visible-area blockers with plate-derived building/prop polygons and runtime-human occupancy, but complete city-wide semantic back-propagation from one authoritative master scene remains open. |
...
| Tokyo city foundation | Requester accepted the current live visual checkpoint; commit closeout active | On 2026-08-05 the requester accepted the people-free matte district, separately rendered street population, larger protagonist, usable zoom-in, reduced glare, building/actor click rejection, and corruption-free overview after desktop/mobile review. The checkpoint uses generated close/overview LOD plates rather than two registered exports from one authoritative Blender master, so GET-204 moves to `In Review`, not `Done`, after the validated commit. |
| Hidzu identity pass | Next approved ticket after the GET-204 acceptance commit | GET-205 commit `7a6bba7508e9062afb2f1cc2521f7c78e5564ae2` retains useful palette, grammar, and generator research, but it was never promoted to the accepted live city. The next pass must apply that identity to the accepted GET-204 visual baseline and return to the requester for its own live acceptance. |
| Street-first camera and composed overview | Live checkpoint accepted; authoritative export/freeze remains open | The accepted checkpoint uses `1.48` normal zoom, `1.84` maximum zoom, a `1.80` close protagonist scale, and viewport cover-fit for the overview. Desktop evidence proves the intended close relationship and uncorrupted zoom transition; the two-plate implementation and oversized mobile HUD prevent treating these values as a final cross-device master-scene contract. |
...
| Graphic surveillance noir | Accepted city baseline; Hidzu-specific pass pending | The live runtime now presents the accepted dense, painterly surveillance-noir city instead of the greybox. Warm practicals, cold institutional masses, wet but reduced-glare streets, grounded separate actors, and a close protagonist establish the baseline. GET-205 still owns explicit Hidzu grammar, schedule-state treatment, and final device/state semantics; surveillance behavior belongs to T8. |
