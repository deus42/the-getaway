...
- The district contains exactly one unarmed patrol drone. During the GET-205 v6 visual rebuild, the presentation also contains exactly one static public restricted-area guard, no static service-entrance guard, exactly three capacity-bound transit passengers, and exactly two seated café patrons, with no random ambient allocation. Their T5 anchors/slots come from [[32 GET-205 Reference-Native Layout Contract]]. Later delivery population, broader human-security/civilian behavior, schedules, camera geometry, and placements remain unresolved by `OPEN-SEC-001`, `OPEN-CIV-001`, and `OPEN-LAYOUT-003` for GET-208.
...
- During the GET-205 v6 visual gate, `blend.public_queue` is populated only by its two seated plus one standing runtime-owned actors and is eligible only while that exact group and its authored schedule are active. Two seated café patrons remain presentation-only at declared furniture. `blend.delivery_activity` is explicitly unavailable because its population is not yet authored. T5 may not fake eligibility, add schedule-unowned figures, exceed visible capacity, or treat the café as generic cover.
- The GET-205 gate is a real dynamic navigation boundary: closed until the existing verification/mission contract authorizes passage and restored closed by Restart Attempt. Barrier, warning lights, signage, one public static guard, and Needle make that boundary visible. GET-208 still owns observed-breach detection, trespass, concern/Pursuit, schedules, interception, and broader verification behavior.
...
- Public activity and security schedules determine the credibility of dusk blending and curfew service movement without arbitrary bonuses. During GET-205 v6, the fixed transit group can support only `blend.public_queue` while visibly present; café patrons provide place and human scale but no blend eligibility, and the absent delivery group keeps `blend.delivery_activity` unavailable. Civilians remain presentation-only mirrors of visible events.
...
- For the GET-205 visual rebuild, remove every random ambient allocation and author only the bounded mission-stage population in `GDR-CIV-003`: two seated plus one standing transit passenger and two seated café patrons, all in exact nonblocking slots. `blend.public_queue` follows its actual group/schedule; `blend.delivery_activity` remains unavailable. Later delivery counts, broader schedules, reactions, group behavior, and blending cadence may use only the replaceable recommendation from `OPEN-CIV-001` until GET-208 acceptance.
...
- No blend context may report eligible while its required authored population is absent. GET-205 cannot replace removed civilians with random, wandering, decorative, baked, capacity-exceeding, or schedule-unowned figures; café patrons never grant generic blending.
- The GET-205 barrier may block/open the direct route only through the existing verification/mission state. It must not imply that observed-breach detection, trespass escalation, human interception, or broader security behavior has been implemented. The unguarded service entrance and permanent bypass remain valid until GET-208 explicitly changes security behavior.
...
- During the GET-205 v6 visual gate, confirm the closed direct gate, authorized opening, Restart Attempt closure, and permanent service bypass; confirm the transit shelter contains exactly two seated plus one standing passenger, the café contains exactly two seated patrons, `blend.public_queue` follows actual occupancy/schedule, `blend.delivery_activity` is unavailable, exactly one public static guard plus Needle remains, and no random or stacked actors appear. GET-208 later adds the delivery group and broader behavior proof.
