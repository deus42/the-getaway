...
- The mandatory reversible baseline is sections 6.1–6.9 of [[32 GET-205 Reference-Native Layout Contract]]. `OPEN-LAYOUT-007` is the requester freeze gate; implementation has no latitude to retain an old coordinate or improvise around the prescribed plan.
...
- The first output is a dimensioned top-down plan, parcel/anchor overlay, and material-free `1440×900` raw greybox with exact mission placeholders. It presents the reference, candidate, masks, and new route/anchor/static/dynamic probe report. The requester performs the blind destination/obstacle/blend/hack/sneak read before seeing the key.
...
- After plan approval, source/facade work adds detailed warm and cold windows, controlled cool-white/cyan fixtures, sparse red threat sources, rich labels, one dominant HIDZU tower, the restricted entrance, and authored transit/café presence. Random ambient allocation remains prohibited.
- The blue-hour hero uses the camera derived and frozen from the v6 plan boxes at `1440×900`, `45°` azimuth, and `30°` elevation, then presents `reference | candidate | delta notes` plus a verified `200%` road/reflection crop. No inherited target or zoom is authoritative.
