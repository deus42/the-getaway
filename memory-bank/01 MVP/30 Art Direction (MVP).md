...
- The first v5 requester-facing output is a material-free greybox at the fixed `1440×900` hero registration with simple, separately owned figure/furniture/device placeholders at mission anchors. It must present `kitbash-reference2-blend-concept-v1.png | candidate massing | building mask/delta`, an occlusion-aware identity segmentation/report, a green route/anchor/probe report, and the candidate/reference building-pixel fraction within the provisional mid-band in `OPEN-ART-006`. The raw frame—not labels alone—must let the requester point to destination, checkpoint obstacle, blend crowd, hackable fixture, and sneaking recess. No facade, authored material, lighting, state bake, cutout, or runtime publication starts before requester approval of this massing.
- The approved massing compacts the four mission blocks into continuous street walls while preserving the validated roads and loops. It uses 12–16 source-traceable building identities, each exported as its own foreground slice with an independent depth anchor; a merged wall cutout is prohibited because it breaks actor sorting at wall ends. At least ten identities must each contribute `0.5%` of the fixed hero frame through occlusion-aware isolation masks, and no identity may contribute more than `15%` of that frame. Hidden declarations and token slivers do not satisfy the gate.
- The tallest, dominant Hidzu identity is placed at `entrance.logistics.public`, with the restricted checkpoint and `camera.public_gate` visibly staged at its base while the protected public road stays clear. The transit shelter/queue, Transit Road café, all three terminal fixtures, all three hide recesses, and Needle's gate-associated patrol volume are composed from their actual anchors rather than added later as decorative facade dressing.
...
- After massing approval, facade work adds distinct identities, more readable warm and cold windows, two to four controlled cool-white/cyan fixtures plus sparse red threat-specific sources, richer labels, the dominant Hidzu destination, and the staged restricted entrance. Amber remains confined to windows, entrances, and lamp falloff. Restricted-area barriers, warnings, signage, one static public guard, and the verifier drone are T5 presentation; detection/trespass behavior remains GET-208-owned. The former service-entrance guard and all random ambient allocation are absent, while the fixed transit/café groups remain separately rendered under `GDR-CIV-003`.
...
8. The requester first accepts the fixed `1440×900` mission-staged greybox with its five-question legibility read, route/anchor/probe report, building-fraction mask, and per-identity contribution report, then the blue-hour hero/reference/delta comparison and `200%` crop, then the complete rebuilt live district. Automated validators and internal stage captures support but never replace those visual gates.
...
- Canonical decisions: `GDR-ART-001` through `GDR-ART-018`, `GDR-CIV-003`, `GDR-SUR-013`, `GDR-UI-001`, `GDR-UI-002`, `GDR-GEO-001`, `GDR-REM-011`, and `GDR-SUP-001` through `GDR-SUP-004` in [[12 Game Design Decision Register]].
